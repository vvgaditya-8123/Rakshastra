"""Shared constants for Rakshastra Agent.
...
_UNSET = object()
_RAKSHASTRA_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_RAKSHASTRA_HOME_OVERRIDE", default=_UNSET
)


def set_rakshastra_home_override(path: str | Path | None) -> Token:
    """Set a context-local Rakshastra home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _RAKSHASTRA_HOME_OVERRIDE.set(value)


def reset_rakshastra_home_override(token: Token) -> None:
    """Restore the previous context-local Rakshastra home override."""
    _RAKSHASTRA_HOME_OVERRIDE.reset(token)


def get_rakshastra_home_override() -> str | None:
    """Return the active context-local Rakshastra home override, if any."""
    override = _RAKSHASTRA_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_rakshastra_home() -> Path:
    """Return the platform-native default Rakshastra home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "rakshastra"
    return Path.home() / ".rakshastra"


def get_rakshastra_home() -> Path:
    """Return the Rakshastra home directory (default: platform-native path).

    Reads RAKSHASTRA_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``RAKSHASTRA_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``RAKSHASTRA_HOME`` explicitly (see the systemd
    template in ``rakshastra_cli/gateway.py`` and the kanban dispatcher in
    ``rakshastra_cli/kanban_db.py``).  See https://github.com/NousResearch/rakshastra-agent/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_rakshastra_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[RAKSHASTRA_HOME fallback] RAKSHASTRA_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass RAKSHASTRA_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_rakshastra_home()


def get_default_rakshastra_root() -> Path:
    """Return the root Rakshastra directory for profile-level operations.

    In standard deployments this is the platform-native Rakshastra home
    (``~/.rakshastra`` on POSIX, ``%LOCALAPPDATA%\\rakshastra`` on native Windows).

    In Docker or custom deployments where ``RAKSHASTRA_HOME`` points outside
    ``~/.rakshastra`` (e.g. ``/opt/data``), returns ``RAKSHASTRA_HOME`` directly
    — that IS the root.

    In profile mode where ``RAKSHASTRA_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.rakshastra/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_rakshastra_home()
    env_home = os.environ.get("RAKSHASTRA_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # RAKSHASTRA_HOME is under ~/.rakshastra (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — RAKSHASTRA_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Rakshastra is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``RAKSHASTRA_OPTIONAL_SKILLS``.
    """
    override = os.getenv("RAKSHASTRA_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_rakshastra_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``RAKSHASTRA_OPTIONAL_MCPS``.
    """
    override = os.getenv("RAKSHASTRA_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_rakshastra_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``RAKSHASTRA_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<RAKSHASTRA_HOME>/skills`` last-resort
    """
    override = os.getenv("RAKSHASTRA_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_rakshastra_home() / "skills"


def get_rakshastra_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Rakshastra subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to RAKSHASTRA_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to RAKSHASTRA_HOME (e.g. ``"image_cache"``).
...
def iter_rakshastra_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Rakshastra-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\rakshastra\\node``. POSIX installs use
    ``$RAKSHASTRA_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_rakshastra_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with rakshastraManagedNodePathEntries() in
...
_RAKSHASTRA_NODE_TARGET_MAJOR = int(os.environ.get("RAKSHASTRA_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Rakshastra-managed Node trees live under ``$RAKSHASTRA_HOME/node`` (or a profile's
    ``RAKSHASTRA_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_rakshastra_node_executable``
    used to trust file presence alone, so ``rakshastra update`` would pick that
...
def rakshastra_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Rakshastra-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_rakshastra_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%RAKSHASTRA_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_rakshastra_home()
    index_url = f"https://nodejs.org/dist/latest-v{_RAKSHASTRA_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_RAKSHASTRA_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_rakshastra_managed_node() -> bool:
    """Redownload Rakshastra-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not rakshastra_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "RAKSHASTRA_HOME": str(get_rakshastra_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_rakshastra_node_executable(command: str) -> str | None:
    """Return a Rakshastra-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_rakshastra_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_rakshastra_managed_node():
        for directory in iter_rakshastra_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Rakshastra-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Rakshastra-managed installs.

    This is for Rakshastra-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_rakshastra_node_executable(command)
    if managed:
        return managed
    if rakshastra_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_rakshastra_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Rakshastra-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_rakshastra_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``rakshastra update`` — leaving a **dangling symlink** that ``which`` still
...
def display_rakshastra_home() -> str:
    """Return a user-friendly display string for the current RAKSHASTRA_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.rakshastra``
        profile:  ``~/.rakshastra/profiles/coder``
        custom:   ``/opt/rakshastra-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.rakshastra``.  For code that needs a real ``Path``, use
    :func:`get_rakshastra_home` instead.
    """
    home = get_rakshastra_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``RAKSHASTRA_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/rakshastra-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{RAKSHASTRA_HOME}/home`` when the profile-home directory exists."""
    rakshastra_home = get_rakshastra_home_override() or (env or {}).get("RAKSHASTRA_HOME") or os.getenv("RAKSHASTRA_HOME")
    if not rakshastra_home:
        return None
    profile_home = os.path.join(rakshastra_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("RAKSHASTRA_REAL_HOME") or os.getenv("RAKSHASTRA_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Rakshastra profile HOME.

    ``RAKSHASTRA_HOME`` scopes Rakshastra state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={RAKSHASTRA_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{RAKSHASTRA_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{RAKSHASTRA_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Rakshastra' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["RAKSHASTRA_REAL_HOME"] = real_home
...
def is_container() -> bool:
    ...
    See: NousResearch/rakshastra-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under RAKSHASTRA_HOME.

    Replaces the ``get_rakshastra_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, rakshastra_logging.py, rakshastra_time.py, etc.).
    """
    return get_rakshastra_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under RAKSHASTRA_HOME."""
    return get_rakshastra_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under RAKSHASTRA_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_rakshastra_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._rakshastra_ipv4_patched = True  # type: ignore[attr-defined]
