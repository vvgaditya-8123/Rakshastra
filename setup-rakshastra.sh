...
# 5. Symlinks the 'rakshastra' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Rakshastra Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/rakshastra-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink rakshastra into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up rakshastra command..."

RAKSHASTRA_BIN="$SCRIPT_DIR/venv/bin/rakshastra"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$RAKSHASTRA_BIN" "$COMMAND_LINK_DIR/rakshastra"
echo -e "${GREEN}✓${NC} Symlinked rakshastra → $COMMAND_LINK_DISPLAY_DIR/rakshastra"
...
# ============================================================================
# Seed bundled skills into ~/.rakshastra/skills/
# ============================================================================

RAKSHASTRA_SKILLS_DIR="${RAKSHASTRA_HOME:-$HOME/.rakshastra}/skills"
mkdir -p "$RAKSHASTRA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.rakshastra/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$RAKSHASTRA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  rakshastra status        # Check configuration"
if is_termux; then
    echo "  rakshastra gateway       # Run gateway in foreground"
else
    echo "  rakshastra gateway install # Install gateway service (messaging + cron)"
fi
echo "  rakshastra cron list     # View scheduled jobs"
echo "  rakshastra doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m rakshastra_cli.main setup
fi
