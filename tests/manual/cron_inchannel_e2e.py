...
Uses a throwaway RAKSHASTRA_HOME so it never touches ~/.rakshastra. No real names.
"""
...
def _fresh_home():
    """Point RAKSHASTRA_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_rakshastra_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["RAKSHASTRA_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER RAKSHASTRA_HOME is set.
...
# a different get_rakshastra_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"RAKSHASTRA_HOME (throwaway): {HOME}")
