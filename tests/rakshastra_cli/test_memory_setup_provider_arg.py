"""Tests for `rakshastra memory setup [provider]` routing.
...
``rakshastra memory setup honcho``) without the interactive picker — which matters
because the per-provider ``rakshastra <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "rakshastra memory setup" in out
