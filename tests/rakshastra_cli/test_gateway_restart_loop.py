...
Covers:
- Defense 1: gateway stop/restart refuse when _RAKSHASTRA_GATEWAY=1
...
from rakshastra_cli.cron import (
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "rakshastra gateway restart",
        "rakshastra gateway stop",
        "rakshastra  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "RAKSHASTRA GATEWAY RESTART",           # uppercase
    ])
    def test_rakshastra_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.rakshastra.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.rakshastra.gateway.plist",
        "launchctl stop ai.rakshastra.gateway",
        "systemctl restart rakshastra-gateway",
        "systemctl stop rakshastra-gateway.service",
        "systemctl start rakshastra-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill rakshastra gateway process",
        "pkill -f rakshastra.*gateway",
        "pkill -f gateway.*rakshastra",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "rakshastra cron list",
        "rakshastra update",
        "rakshastra config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `rakshastra gateway start` is benign — starting a gateway from inside a
        ...
        "rakshastra gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway rakshastra
        # services must not be falsely blocked (the old `.*rakshastra` matched any
        # rakshastra token).
        "launchctl unload ai.rakshastra.update-checker.plist",
        "launchctl restart ai.rakshastra.daemon",
        "systemctl restart rakshastra-meta.service",
        "systemctl restart rakshastra-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_rakshastra_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade rakshastra then run rakshastra gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.rakshastra.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_rakshastra_gateway_once.sh). The script must live under
        # RAKSHASTRA_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("RAKSHASTRA_HOME", str(tmp_path / ".rakshastra"))
        scripts_dir = tmp_path / ".rakshastra" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify rakshastra gateway stop/restart refuse when _RAKSHASTRA_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_RAKSHASTRA_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_RAKSHASTRA_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _RAKSHASTRA_GATEWAY=1.

    Issue #37453: systemctl --user restart rakshastra-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart rakshastra-gateway",
        "systemctl --user restart rakshastra-gateway",
        "systemctl stop rakshastra-gateway.service",
        "rakshastra gateway restart",
        "launchctl kickstart gui/501/ai.rakshastra.gateway",
        "pkill -f rakshastra.*gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart rakshastra-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-rakshastra systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _RAKSHASTRA_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart rakshastra-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart rakshastra-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run rakshastra gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("rakshastra gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xferakshastra gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under RAKSHASTRA_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.rakshastra.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    rakshastra_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run rakshastra gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("RAKSHASTRA_HOME", str(tmp_path / ".rakshastra"))
        (tmp_path / ".rakshastra").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run rakshastra gateway restart nightly",
        ))
