...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    RAKSHASTRA_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_rakshastra_argv", lambda: ["rakshastra"])
    ...
    assert pid == 4242
    assert captured["env"]["RAKSHASTRA_HOME"] == str(profile)
    assert captured["env"]["RAKSHASTRA_KANBAN_TASK"] == "t_spawn_tools"
