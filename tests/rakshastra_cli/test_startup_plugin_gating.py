...
``rakshastra_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``rakshastra --help``, ``rakshastra version``,
``rakshastra logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``rakshastra --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from rakshastra_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["rakshastra", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        (["rakshastra"], None),
        (["rakshastra", "--help"], None),
        (["rakshastra", "-h"], None),
        (["rakshastra", "--version"], None),
        (["rakshastra", "-w"], None),
        ...
        (["rakshastra", "-w", "--tui"], None),
        (["rakshastra", "version"], "version"),
        (["rakshastra", "--tui", "chat"], "chat"),
        (["rakshastra", "-w", "logs"], "logs"),
        (["rakshastra", "chat", "hello world"], "chat"),
        (["rakshastra", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["rakshastra", "-m", "gpt5", "chat"], "chat"),
        (["rakshastra", "--model", "gpt5", "chat", "hi"], "chat"),
        (["rakshastra", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["rakshastra", "-z", "hello world"], None),
        (["rakshastra", "-z", "hello", "chat"], "chat"),
        (["rakshastra", "--model=gpt5", "chat"], "chat"),     # inline form
        (["rakshastra", "--", "chat"], "chat"),               # -- terminator
        (["rakshastra", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["rakshastra", "some-plugin-cmd"], "some-plugin-cmd"),
        (["rakshastra", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["rakshastra"],                          # bare → chat
        ["rakshastra", "--help"],                # top-level help
        ["rakshastra", "-h"],
        ["rakshastra", "version"],               # known built-in
        ["rakshastra", "logs"],
        ["rakshastra", "gateway", "run"],
        ["rakshastra", "--tui"],
        ["rakshastra", "-w", "--tui"],
        ["rakshastra", "chat", "hi"],
        ["rakshastra", "help"],                  # accepted built-in-ish
        ["rakshastra", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["rakshastra", "meet", "join"],          # potential google_meet plugin
        ["rakshastra", "honcho", "status"],      # potential memory plugin
        ["rakshastra", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``rakshastra help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"rakshastra_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
