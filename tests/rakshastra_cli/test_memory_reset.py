"""Tests for the `rakshastra memory reset` CLI command.
...
- Profile-scoped reset (uses RAKSHASTRA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake RAKSHASTRA_HOME with memory files."""
    rakshastra_home = tmp_path / ".rakshastra"
    memories = rakshastra_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("RAKSHASTRA_HOME", str(rakshastra_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nRakshastra repo is at ~/.rakshastra/rakshastra-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
    ...
    return rakshastra_home, memories
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `rakshastra memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        rakshastra_home = tmp_path / ".rakshastra"
        (rakshastra_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("RAKSHASTRA_HOME", str(rakshastra_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's RAKSHASTRA_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_rakshastra_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
