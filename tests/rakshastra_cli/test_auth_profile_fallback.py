...
When ``RAKSHASTRA_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.rakshastra/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.rakshastra            (has its own auth.json fixture)
    * Profile     -> tmp_path/.rakshastra/profiles/coder   (active, RAKSHASTRA_HOME points here)
    ...
    global_root = tmp_path / ".rakshastra"
    global_root.mkdir()
...
# ``_load_provider_state`` itself, those helpers raise ``"Rakshastra is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from rakshastra_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    rakshastra_home = tmp_path / "classic"
    rakshastra_home.mkdir()
    monkeypatch.setenv("RAKSHASTRA_HOME", str(rakshastra_home))

    _write(rakshastra_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (RAKSHASTRA_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.rakshastra as the "real home" — which is NOT equal
    # to the RAKSHASTRA_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    rakshastra_home = tmp_path / "classic"
    ...
    _write(rakshastra_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from rakshastra_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: RAKSHASTRA_HOME is set to a custom path that is NOT under
    # ~/.rakshastra/profiles/ — get_default_rakshastra_root() returns RAKSHASTRA_HOME
