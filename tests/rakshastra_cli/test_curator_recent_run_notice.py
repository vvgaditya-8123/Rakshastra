...
The notice prints the most recent curator run summary on `rakshastra update`,
...
session start) so users normally never see the rename map. `rakshastra update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(rakshastra_main)

    yield {
        "curator": curator,
        "main": rakshastra_main,
        "capsys": capsys,
    }
