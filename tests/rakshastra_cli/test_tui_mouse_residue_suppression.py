...
The Python launcher (`rakshastra --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Rakshastra session
...
from rakshastra_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["rakshastra", "--tui", "-c", "abc"])
        monkeypatch.delenv("RAKSHASTRA_TUI", raising=False)
        monkeypatch.delenv("RAKSHASTRA_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_writes_disable_sequence_when_rakshastra_tui_env_set(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["rakshastra"])
        monkeypatch.setenv("RAKSHASTRA_TUI", "1")
        monkeypatch.delenv("RAKSHASTRA_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.delenv("RAKSHASTRA_TUI", raising=False)
        monkeypatch.setenv("RAKSHASTRA_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `rakshastra --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
