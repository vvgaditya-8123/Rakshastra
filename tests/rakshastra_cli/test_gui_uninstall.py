"""Tests for rakshastra_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import rakshastra_cli.gui_uninstall as gu


def _make_agent(rakshastra_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = rakshastra_home / "rakshastra-agent"
    (agent_root / "rakshastra_cli").mkdir(parents=True)
    (agent_root / "rakshastra_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(rakshastra_home: Path) -> None:
    """Create the source-built GUI artifacts a `rakshastra desktop` run produces."""
    desktop = rakshastra_home / "rakshastra-agent" / "apps" / "desktop"
    ...
    (rakshastra_home / "rakshastra-agent" / "node_modules").mkdir(parents=True)
    (rakshastra_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(rakshastra_home: Path) -> None:
    (rakshastra_home / "config.yaml").write_text("x: 1\n")
    (rakshastra_home / ".env").write_text("KEY=secret\n")
    (rakshastra_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    rakshastra_home = tmp_path / ".rakshastra"
    rakshastra_home.mkdir()
    assert gu.agent_is_installed(rakshastra_home) is False
    _make_agent(rakshastra_home)
    assert gu.agent_is_installed(rakshastra_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    rakshastra_home = tmp_path / ".rakshastra"
    (rakshastra_home / "rakshastra-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(rakshastra_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    rakshastra_home = tmp_path / ".rakshastra"
    _make_gui_build(rakshastra_home)
    artifacts = gu.source_built_gui_artifacts(rakshastra_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(rakshastra_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(rakshastra_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    rakshastra_home = tmp_path / ".rakshastra"
    agent_root = _make_agent(rakshastra_home)
    _make_gui_build(rakshastra_home)
    _make_user_data(rakshastra_home)
    ...
    removed = gu.uninstall_gui(rakshastra_home)
    ...
    assert not (rakshastra_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "rakshastra_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (rakshastra_home / "config.yaml").exists()
    assert (rakshastra_home / ".env").exists()
    assert (rakshastra_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    rakshastra_home = tmp_path / ".rakshastra"
    _make_agent(rakshastra_home)
    userdata = tmp_path / "Rakshastra-userdata"
    userdata.mkdir()
    ...
    gu.uninstall_gui(rakshastra_home)
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(rakshastra_home, remove_userdata=False)
...
def test_uninstall_gui_removes_packaged_bundle(tmp_path, monkeypatch):
    rakshastra_home = tmp_path / ".rakshastra"
    _make_agent(rakshastra_home)
    bundle = tmp_path / "Rakshastra.app"
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(rakshastra_home)
    ...
    assert summary["rakshastra_home"] == str(rakshastra_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Rakshastra"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Rakshastra"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Rakshastra"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Rakshastra"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    RAKSHASTRA_HOME with all the destructive externals stubbed out.
    """
    import rakshastra_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_rakshastra_home", lambda: rakshastra_home)
    ...
    from rakshastra_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (rakshastra_home / "config.yaml").exists()
    assert rakshastra_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole RAKSHASTRA_HOME non-interactively."""
    import rakshastra_cli.uninstall as uninstall

    rakshastra_home = tmp_path / ".rakshastra"
    (rakshastra_home / "rakshastra-agent" / "rakshastra_cli").mkdir(parents=True)
    (rakshastra_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not rakshastra_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m rakshastra_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_rakshastra_home", lambda: rakshastra_home)
    ...
    assert not (desktop / "release").exists()
    assert not (rakshastra_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "rakshastra_cli").exists()
    assert (rakshastra_home / "config.yaml").exists()
