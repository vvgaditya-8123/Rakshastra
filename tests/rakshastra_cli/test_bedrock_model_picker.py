...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (RAKSHASTRA_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via RAKSHASTRA_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in RAKSHASTRA_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from rakshastra_cli.providers import RAKSHASTRA_OVERLAYS
        assert "bedrock" in RAKSHASTRA_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from rakshastra_cli.providers import RAKSHASTRA_OVERLAYS
        assert RAKSHASTRA_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from rakshastra_cli.providers import RAKSHASTRA_OVERLAYS
        assert RAKSHASTRA_OVERLAYS["bedrock"].auth_type == "aws_sdk"
