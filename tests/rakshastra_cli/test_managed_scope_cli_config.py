...
rakshastra_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `rakshastra config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_rakshastra_home = get_rakshastra_home()`` at import time (module
    singleton), so monkeypatching RAKSHASTRA_HOME after import doesn't move it.
    ...
    this only matters for tests that swap RAKSHASTRA_HOME.)
    """
    import cli

    cli._rakshastra_home = home
