...
def container_env(tmp_path, monkeypatch):
    """Set up a fake RAKSHASTRA_HOME with .container-mode file."""
    rakshastra_home = tmp_path / ".rakshastra"
    rakshastra_home.mkdir()
    monkeypatch.setenv("RAKSHASTRA_HOME", str(rakshastra_home))
    monkeypatch.delenv("RAKSHASTRA_DEV", raising=False)

    container_mode = rakshastra_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=rakshastra-agent\n"
        "exec_user=rakshastra\n"
        "rakshastra_bin=/data/current-package/bin/rakshastra\n"
    )
    return rakshastra_home


def test_get_container_exec_info_returns_metadata(container_env):
    """Reads .container-mode and returns all fields including exec_user."""
    with patch("rakshastra_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "rakshastra-agent"
    assert info["exec_user"] == "rakshastra"
    assert info["rakshastra_bin"] == "/data/current-package/bin/rakshastra"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("rakshastra_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_rakshastra_dev(container_env, monkeypatch):
    """Returns None when RAKSHASTRA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("RAKSHASTRA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_rakshastra_dev_zero(container_env, monkeypatch):
    """RAKSHASTRA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("RAKSHASTRA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        rakshastra_home = Path(tmpdir) / ".rakshastra"
        rakshastra_home.mkdir()
        (rakshastra_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("rakshastra_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_rakshastra_home": lambda: rakshastra_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("RAKSHASTRA_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=rakshastra-custom\n"
        "exec_user=myuser\n"
        "rakshastra_bin=/opt/rakshastra/bin/rakshastra\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "rakshastra-custom"
    assert info["exec_user"] == "myuser"
    assert info["rakshastra_bin"] == "/opt/rakshastra/bin/rakshastra"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from rakshastra_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "rakshastra"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "rakshastra-agent" in cmd
    assert "/data/current-package/bin/rakshastra" in cmd
    assert "chat" in cmd
