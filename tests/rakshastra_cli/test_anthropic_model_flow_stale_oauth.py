"""Tests for Bug #12905 fix — stale OAuth token detection in rakshastra model flow.

Bug 3: `rakshastra model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.rakshastra/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("rakshastra_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from rakshastra_cli.main import _model_flow_anthropic
        cfg = {}
