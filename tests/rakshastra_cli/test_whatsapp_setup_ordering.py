...
Before the fix, ``rakshastra whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``rakshastra gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    rakshastra = home / ".rakshastra"
    rakshastra.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("RAKSHASTRA_HOME", str(rakshastra))
...
def _env_value(rakshastra_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from rakshastra_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("rakshastra_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``rakshastra whatsapp`` with an existing paired session and
