"""Tests for --ignore-user-config and --ignore-rules flags on `rakshastra chat`.
...
files. In Rakshastra the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.rakshastra/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → RakshastraCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("RAKSHASTRA_IGNORE_USER_CONFIG", "RAKSHASTRA_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``RAKSHASTRA_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<rakshastra_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._rakshastra_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_user_config_skipped_when_flag_set(self, tmp_path, monkeypatch):
        """With RAKSHASTRA_IGNORE_USER_CONFIG=1, user config.yaml is ignored.

        The built-in default ``model.default`` is empty string (no user override),
        and the user's ``agent.system_prompt`` is not seen.
        """
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("RAKSHASTRA_IGNORE_USER_CONFIG", "1")
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("RAKSHASTRA_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``RakshastraCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting RAKSHASTRA_IGNORE_RULES=1 flips RakshastraCLI.ignore_rules True."""
        monkeypatch.setenv("RAKSHASTRA_IGNORE_RULES", "1")

        # Import RakshastraCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of RakshastraCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.RakshastraCLI)
        # Replicate the exact logic from cli.py RakshastraCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("RAKSHASTRA_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("RAKSHASTRA_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.RakshastraCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``rakshastra_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("RAKSHASTRA_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("RAKSHASTRA_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("RAKSHASTRA_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("RAKSHASTRA_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("RAKSHASTRA_IGNORE_USER_CONFIG") == "1"
        assert "RAKSHASTRA_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "RAKSHASTRA_IGNORE_USER_CONFIG" not in os.environ
        assert "RAKSHASTRA_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_flags_present_in_chat_parser(self):
        ...
        import argparse
        parser = argparse.ArgumentParser(prog="rakshastra")
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real rakshastra parser accepts both flags."""
        ...
        import inspect
        import rakshastra_cli.main as hm
        src = inspect.getsource(hm)
        assert "RAKSHASTRA_IGNORE_USER_CONFIG" in src
        assert "RAKSHASTRA_IGNORE_RULES" in src
