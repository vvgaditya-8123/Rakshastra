"""Tests for the per-profile Projects store (rakshastra_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Rakshastra Agent", folders=["/tmp/rakshastra"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "rakshastra-agent"
    assert proj.name == "Rakshastra Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/rakshastra"
    assert [f.path for f in proj.folders] == ["/tmp/rakshastra"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "rakshastra-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Rakshastra Agent")
    pdb.create_project(conn, name="Rakshastra Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["rakshastra-agent", "rakshastra-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' RAKSHASTRA_HOME.
...
def test_db_path_under_rakshastra_home():
    # Resolves under RAKSHASTRA_HOME (set by the autouse isolation fixture).
