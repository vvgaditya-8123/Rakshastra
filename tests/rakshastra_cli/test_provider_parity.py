...
the SAME provider universe as ``rakshastra model`` (the CLI/TUI picker).
...
def test_every_rakshastra_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `rakshastra model` universe."""
    ...
    assert not missing, (
        "providers shown in `rakshastra model` but not configurable in the desktop "
