...
# ── deprecated `rakshastra login` fails gracefully, not with argparse error ────
#
# `rakshastra login` is a removed command; its handler (`login_command` in
# `rakshastra_cli/auth.py`) prints a deprecation notice pointing at `rakshastra auth` /
# `rakshastra model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `rakshastra --help`.
