"""Tests for IRC gateway configuration via `rakshastra setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Rakshastra install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Rakshastra install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#rakshastra")
            monkeypatch.setenv("IRC_NICKNAME", "rakshastra-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `rakshastra setup gateway` experience with IRC present."""
