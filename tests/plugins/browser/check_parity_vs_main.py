...
Run from the PR worktree:
    ...
    cd ~/.rakshastra/rakshastra-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.rakshastra/rakshastra-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.rakshastra/rakshastra-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a rakshastra-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a rakshastra-agent checkout"
)
...
# Isolated RAKSHASTRA_HOME for the config write.
