...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate RAKSHASTRA_HOME under tmp_path."""
    monkeypatch.setenv("RAKSHASTRA_HOME", str(tmp_path / ".rakshastra"))
    (tmp_path / ".rakshastra").mkdir()

    # Force the constants/image cache helpers to re-read RAKSHASTRA_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("rakshastra_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_rakshastra_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under RAKSHASTRA_HOME — gateway cleanup
