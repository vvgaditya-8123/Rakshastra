...
""",
        ...
        from rakshastra_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        rakshastra_home = tmp_path / ".rakshastra"
        rakshastra_home.mkdir()
        (rakshastra_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("RAKSHASTRA_HOME", str(rakshastra_home))
        from rakshastra_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from rakshastra_cli.plugins import PluginContext, PluginManifest, PluginManager
