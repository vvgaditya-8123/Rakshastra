...
engine plugins (e.g. rakshastra-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``rakshastra plugins`` picker."""
    from rakshastra_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "rakshastra_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "rakshastra_cli.plugins.get_plugin_context_engine",
