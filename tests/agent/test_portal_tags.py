...
def test_rakshastra_client_tag_includes_current_version():
    """The client tag must reflect rakshastra_cli.__version__ verbatim."""
    from rakshastra_cli import __version__
    from agent.portal_tags import rakshastra_client_tag

    assert rakshastra_client_tag() == f"client=rakshastra-client-v{__version__}"


def test_rakshastra_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import rakshastra_client_tag

    tag = rakshastra_client_tag()
    assert tag.startswith("client=rakshastra-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import rakshastra_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=rakshastra-agent" in tags
    assert rakshastra_client_tag() in tags
    assert len(tags) == 2
