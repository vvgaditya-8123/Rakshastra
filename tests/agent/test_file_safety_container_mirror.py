...
``…/sandboxes/<backend>/<task>/home/.rakshastra/…`` prefix. This covers the
complementary inner-container case: when file tools execute inside Docker,
the bind-mount strips that prefix and the guard sees plain ``/root/.rakshastra/…``.
...
class TestClassifyContainerMirrorTarget:
    def test_returns_none_without_context(self):
        """No Docker context — /root/.rakshastra/… must not be flagged."""
        from agent.file_safety import classify_container_mirror_target

        assert classify_container_mirror_target("/root/.rakshastra/profiles/group1/SOUL.md") is None
    ...
    def test_catches_soul_md_with_context(self):
        ...
        result = classify_container_mirror_target(
            "/root/.rakshastra/profiles/group1/SOUL.md",
            mirror_prefix="/root/.rakshastra",
        )
        assert result is not None
        assert result["mirror_root"].replace("\\", "/").endswith("root/.rakshastra")
    ...
    def test_catches_authoritative_profile_files(self, inner):
        ...
        result = classify_container_mirror_target(
            f"/root/.rakshastra/{inner}",
            mirror_prefix="/root/.rakshastra",
        )
        assert result is not None
        assert result["inner_path"] == inner

    def test_non_rakshastra_path_not_flagged(self):
        """/root/workspace/… is not .rakshastra state and must not be blocked."""
        from agent.file_safety import classify_container_mirror_target
...
class TestOrthogonality:
    ...
    def test_inner_container_path_caught_by_context_guard(self):
        ...
        path = "/root/.rakshastra/profiles/group1/SOUL.md"

        assert classify_container_mirror_target(path) is None  # no context
        assert classify_container_mirror_target(path, mirror_prefix="/root/.rakshastra") is not None
...
class TestFileToolIntegration:
    ...
    def test_guard_uses_current_docker_config_before_env_exists(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda task_id: "/root/.rakshastra",
        )
