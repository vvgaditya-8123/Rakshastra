...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated RAKSHASTRA_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="rakshastra_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".rakshastra"))
    prev_home = os.environ.get("RAKSHASTRA_HOME")
    os.environ["RAKSHASTRA_HOME"] = os.path.join(test_home, ".rakshastra")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("rakshastra_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["RAKSHASTRA_HOME"] = prev_home
