...
def isolated_home(monkeypatch):
    """Temp RAKSHASTRA_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="rakshastra_test_31179_")
    rakshastra_home = os.path.join(test_home, ".rakshastra")
    os.makedirs(rakshastra_home)
...
def _fresh_modules():
    """Drop cached rakshastra modules so each test reloads against current env."""
