...
path via ``rakshastra chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp RAKSHASTRA_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point RAKSHASTRA_HOME at a temp dir and turn moa.save_traces on."""
    rakshastra_home = tmp_path / ".rakshastra"
    rakshastra_home.mkdir()
    monkeypatch.setenv("RAKSHASTRA_HOME", str(rakshastra_home))

    # save_moa_turn reads config via rakshastra_cli.config.load_config; stub it to
    ...
    return rakshastra_home / "moa-traces"
