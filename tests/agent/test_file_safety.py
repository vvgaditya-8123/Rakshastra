...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_rakshastra_env(self):
        """Rakshastra' own .env inside RAKSHASTRA_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: rakshastra internal .env is in ~/.rakshastra/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.rakshastra/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.rakshastra/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Rakshastra cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        rakshastra_home = tmp_path / ".rakshastra"
        cache = rakshastra_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._rakshastra_home_path", return_value=rakshastra_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Rakshastra cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        rakshastra_home = tmp_path / ".rakshastra"
        hub = rakshastra_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_rakshastra_home(self, tmp_path):
        """The env basename guard does not depend on RAKSHASTRA_HOME resolution."""
        rakshastra_home = tmp_path / ".rakshastra"
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        rakshastra_home = tmp_path / ".rakshastra"
        cache = rakshastra_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
