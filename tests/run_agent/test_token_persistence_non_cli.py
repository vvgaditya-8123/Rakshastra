...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    rakshastra_state = ModuleType("rakshastra_state")
    rakshastra_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "rakshastra_state", rakshastra_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Rakshastra"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Rakshastra"
