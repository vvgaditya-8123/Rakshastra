"""Tests for rakshastra_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Rakshastra entry point
(rakshastra, rakshastra-agent, rakshastra-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Rakshastra entry point imports rakshastra_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported rakshastra_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("rakshastra_bootstrap", None)
    import rakshastra_bootstrap  # noqa: WPS433
    return rakshastra_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Rakshastra CLI
...
class TestEntryPointsImportBootstrap:
    """Every Rakshastra entry point must import rakshastra_bootstrap as its
    ...
    # Entry points that invoke Rakshastra as a process.  Each one must
    # import rakshastra_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "rakshastra_cli/main.py",   # rakshastra CLI (console_script)
        "run_agent.py",          # rakshastra-agent (console_script)
        "acp_adapter/entry.py",  # rakshastra-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import rakshastra_bootstrap' and that
        ...
        half-finished ``rakshastra update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``rakshastra_bootstrap`` as a top-level module) leaves rakshastra
        ...
        """
        # Resolve relative to the rakshastra-agent repo root.  Tests live
        # at tests/test_rakshastra_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # rakshastra start even when rakshastra_bootstrap hasn't been
        ...
        assert first_import_name == "rakshastra_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'rakshastra_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import rakshastra_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Rakshastra's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("RAKSHASTRA_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["RAKSHASTRA_PYTHON_SRC_ROOT"] = env
            elif "RAKSHASTRA_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["RAKSHASTRA_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/rakshastra")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("RAKSHASTRA_PYTHON_SRC_ROOT", None)
            else:
                os.environ["RAKSHASTRA_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/rakshastra", "/usr/lib/python"])
    ...
    def test_src_root_forced_to_front(self):
        hb = _fresh_import()
        result = self._run(hb, ["", "/opt/rakshastra", "/usr/lib/python"])
        assert result[0] == "/opt/rakshastra"
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Rakshastra root.  The guard must relocate Rakshastra to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/rakshastra"])
        assert result[0] == "/opt/rakshastra"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Rakshastra root.
        assert result.index("/opt/rakshastra") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/rakshastra", "/opt/rakshastra", ""])
        assert result.count("/opt/rakshastra") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["RAKSHASTRA_PYTHON_SRC_ROOT"] = "/env/rakshastra"
            hb.harden_import_path()
            assert sys.path[0] == "/env/rakshastra"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("RAKSHASTRA_PYTHON_SRC_ROOT", None)
