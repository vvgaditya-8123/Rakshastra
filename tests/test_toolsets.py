...
class TestToolsetConsistency:
    ...
    def test_rakshastra_platforms_share_core_tools(self):
        """All rakshastra-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on rakshastra-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["rakshastra-cli", "rakshastra-telegram", "rakshastra-discord", "rakshastra-whatsapp", "rakshastra-slack", "rakshastra-signal", "rakshastra-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_rakshastra_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("rakshastra-whatsapp")

    def test_rakshastra_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("rakshastra-api-server")
