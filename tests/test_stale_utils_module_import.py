...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.rakshastra/rakshastra-agent/utils.py)
...
2. ``rakshastra update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``rakshastra_cli/main.py`` (the ``rakshastra update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``rakshastra_constants`` after the
