...
def server():
    with patch.dict("sys.modules", {
        "rakshastra_constants": MagicMock(get_rakshastra_home=MagicMock(return_value="/tmp/rakshastra_test")),
        "rakshastra_cli.env_loader": MagicMock(),
        "rakshastra_cli.banner": MagicMock(),
        "rakshastra_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `RAKSHASTRA_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("RAKSHASTRA_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("RAKSHASTRA_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_sync_session_key_after_compress_reanchors_active_session_lease(
    ...
    from rakshastra_cli.active_sessions import (
...
def test_config_load_missing(server, tmp_path):
    server._rakshastra_home = tmp_path
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/rakshastra-agent-dev": {"name": "rakshastra-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "rakshastra-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "rakshastra_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "rakshastra-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "rakshastra-agent-dev"
