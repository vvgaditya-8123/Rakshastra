...
def server():
    with patch.dict(
        "sys.modules",
        {
            "rakshastra_constants": MagicMock(
                get_rakshastra_home=MagicMock(return_value="/tmp/rakshastra_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'rakshastra-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'rakshastra-release' patched"
    }
