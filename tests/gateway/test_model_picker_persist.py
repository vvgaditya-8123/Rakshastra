...
closure the PR changed, against a real temp ``RAKSHASTRA_HOME``.
"""
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "rakshastra_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_rakshastra_home() / config.yaml`` — point it here.
    monkeypatch.setattr("rakshastra_constants.get_rakshastra_home", lambda: rakshastra_home)
    monkeypatch.setattr("rakshastra_cli.config.get_rakshastra_home", lambda: rakshastra_home)
    return cfg_path
