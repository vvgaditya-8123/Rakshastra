...
def test_mention_patterns_list_matches():
    adapter = _make_adapter(mention_patterns=["hey rakshastra", "rakshastra,"])
    assert adapter._slack_message_matches_mention_patterns("hey rakshastra, you there?") is True
    assert adapter._slack_message_matches_mention_patterns("just chatting") is False


def test_mention_patterns_case_insensitive():
    adapter = _make_adapter(mention_patterns=["hey rakshastra"])
    assert adapter._slack_message_matches_mention_patterns("HEY RAKSHASTRA!") is True


def test_mention_patterns_single_string():
    adapter = _make_adapter(mention_patterns="^rakshastra")
    assert adapter._slack_message_matches_mention_patterns("rakshastra do this") is True
    assert adapter._slack_message_matches_mention_patterns("ok rakshastra") is False


def test_mention_patterns_invalid_regex_skipped_without_crash():
    # An invalid pattern is dropped; valid siblings still work.
    adapter = _make_adapter(mention_patterns=["(unclosed", "hey rakshastra"])
    assert adapter._slack_message_matches_mention_patterns("hey rakshastra") is True


def test_mention_patterns_env_var_fallback(monkeypatch):
    monkeypatch.setenv("SLACK_MENTION_PATTERNS", '["hey rakshastra", "rakshastra,"]')
    adapter = _make_adapter()  # no config value -> falls back to env
    assert adapter._slack_message_matches_mention_patterns("hey rakshastra") is True
...
def test_mention_patterns_env_var_csv_fallback_splits_patterns(monkeypatch):
    ...
    assert [pattern.pattern for pattern in patterns] == ["hey rakshastra", "rakshastra"]
    assert adapter._slack_message_matches_mention_patterns("hey rakshastra") is True
...
def test_mention_patterns_trigger_in_channel_without_literal_mention():
    ...
    adapter = _make_adapter(require_mention=True, mention_patterns=["hey rakshastra"])
    assert _would_process(adapter, text="hey rakshastra what's the status") is True
