...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "rakshastra_cli"}
    ...
    try:
        ...
        _reset_modules(("tools", "agent", "rakshastra_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    sys.modules["rakshastra_cli.config"] = types.SimpleNamespace(
        get_rakshastra_home=lambda: Path(tempfile.gettempdir()) / "rakshastra-home",
    )
...
def test_managed_modal_rejects_host_credential_passthrough():
    _install_fake_tools_package(
        credential_mounts=[{
            ...
            "container_path": "/root/.rakshastra/token.json",
        }]
    )
    managed_modal = _load_tool_module("tools.environments.managed_modal", "environments/managed_modal.py")
