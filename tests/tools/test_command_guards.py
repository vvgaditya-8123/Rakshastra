...
def _clean_state():
    ...
    saved = {}
    for k in ("RAKSHASTRA_INTERACTIVE", "RAKSHASTRA_GATEWAY_SESSION", "RAKSHASTRA_EXEC_ASK", "RAKSHASTRA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["RAKSHASTRA_INTERACTIVE"] = "1"
        session_key = os.getenv("RAKSHASTRA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No RAKSHASTRA_INTERACTIVE or RAKSHASTRA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
