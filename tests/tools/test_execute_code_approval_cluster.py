...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad RAKSHASTRA_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: RAKSHASTRA_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("RAKSHASTRA_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("RAKSHASTRA_CRON_SESSION", "1")
    monkeypatch.delenv("RAKSHASTRA_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_rakshastra_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "RAKSHASTRA_HOME": "/h", "RAKSHASTRA_PROFILE": "p",
        "RAKSHASTRA_CONFIG": "/c.yaml", "RAKSHASTRA_ENV": "/e",
        # other RAKSHASTRA_* → dropped (broad prefix removed)
        "RAKSHASTRA_BASE_URL": "https://x", "RAKSHASTRA_INTERACTIVE": "1",
        "RAKSHASTRA_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("RAKSHASTRA_HOME", "RAKSHASTRA_PROFILE", "RAKSHASTRA_CONFIG", "RAKSHASTRA_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "RAKSHASTRA_BASE_URL", "RAKSHASTRA_INTERACTIVE", "RAKSHASTRA_KANBAN_DB",
...
def test_env_scrub_logs_dropped_rakshastra_vars(caplog):
    """Dropping a non-allowlisted, non-secret RAKSHASTRA_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset RAKSHASTRA_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "RAKSHASTRA_HOME": "/h",          # allowlisted → kept, not logged
        "RAKSHASTRA_BASE_URL": "https://x",   # dropped → logged
        "RAKSHASTRA_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "RAKSHASTRA_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "RAKSHASTRA_HOME" in out and "PATH" in out
    assert "RAKSHASTRA_BASE_URL" not in out and "RAKSHASTRA_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "RAKSHASTRA_BASE_URL" in msgs and "RAKSHASTRA_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "RAKSHASTRA_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped RAKSHASTRA_* vars."""
    import logging
    ...
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        _scrub_child_env(
            {"RAKSHASTRA_HOME": "/h", "PATH": "/usr/bin"},
