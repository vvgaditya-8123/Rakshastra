...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._rakshastra_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(rakshastra_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(rakshastra_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_rakshastra_bin(self):
        """After _INSTALL_FAILED, manual install in RAKSHASTRA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        rakshastra_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(rakshastra_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(rakshastra_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._rakshastra_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == rakshastra_bin
            assert _tirith_mod._resolved_path == rakshastra_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# RAKSHASTRA_HOME isolation
# ---------------------------------------------------------------------------

class TestRakshastraHomeIsolation:
    def test_rakshastra_bin_dir_respects_rakshastra_home(self):
        """_rakshastra_bin_dir must use RAKSHASTRA_HOME, not hardcoded ~/.rakshastra."""
        from tools.tirith_security import _rakshastra_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"RAKSHASTRA_HOME": tmpdir}):
            result = _rakshastra_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_rakshastra_home(self):
        """_failure_marker_path must use RAKSHASTRA_HOME, not hardcoded ~/.rakshastra."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"RAKSHASTRA_HOME": "/custom/rakshastra"}):
            result = _failure_marker_path()
        assert result == "/custom/rakshastra/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets RAKSHASTRA_HOME; verify it's active."""
        rakshastra_home = os.getenv("RAKSHASTRA_HOME")
        assert rakshastra_home is not None, "RAKSHASTRA_HOME should be set by conftest"
        assert "rakshastra_test" in rakshastra_home, "Should point to test temp dir"

    def test_get_rakshastra_home_fallback(self):
        """Without RAKSHASTRA_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_rakshastra_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove RAKSHASTRA_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".rakshastra")
            result = _get_rakshastra_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._rakshastra_bin_dir",
