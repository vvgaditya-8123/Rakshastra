...
def test_rakshastra_provider_subclass_exists():
    """RakshastraMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _RAKSHASTRA_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _RAKSHASTRA_PROVIDER_CLS is not None
    assert issubclass(_RAKSHASTRA_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_rakshastra_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns RakshastraMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _RAKSHASTRA_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("RAKSHASTRA_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _RAKSHASTRA_PROVIDER_CLS is not None
    assert isinstance(provider, _RAKSHASTRA_PROVIDER_CLS)
    assert provider._rakshastra_server_name == "srv"
