...
Older images made /opt/rakshastra/ui-tui and /opt/rakshastra/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV RAKSHASTRA_TUI_DIR=/opt/rakshastra/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R rakshastra:rakshastra /opt/rakshastra/ui-tui" not in text
    assert "chown -R rakshastra:rakshastra /opt/rakshastra/node_modules" not in text
