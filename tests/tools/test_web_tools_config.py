...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_rakshastra_home_override(self, tmp_path):
        """Auth lookup should read from RAKSHASTRA_HOME/auth.json, not ~/.rakshastra/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".rakshastra").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "RAKSHASTRA_HOME": str(rakshastra_home),
        }, clear=False):
            import tools.web_tools
...
class TestBackendSelection:
    ...
    ``rakshastra tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "rakshastra_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
