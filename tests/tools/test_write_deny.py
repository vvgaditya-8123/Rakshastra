...
class TestWriteDenyExactPaths:
    ...
    def test_rakshastra_env(self):
        # ``.env`` under the active RAKSHASTRA_HOME (profile-aware, not just
        # ``~/.rakshastra``) must be write-denied. The hermetic test conftest
        # points RAKSHASTRA_HOME at a tempdir — resolve via get_rakshastra_home()
        # to match the denylist.
        from rakshastra_constants import get_rakshastra_home
        path = str(get_rakshastra_home() / ".env")
        assert _is_write_denied(path) is True

    def test_rakshastra_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.rakshastra/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "rakshastra_root"
        ...
        # Sanity check: RAKSHASTRA_HOME does point to the profile dir, not the root.
        from rakshastra_constants import get_rakshastra_home, get_default_rakshastra_root
        assert get_rakshastra_home() == profile_home
        assert get_default_rakshastra_root() == root
...
class TestWriteAllowed:
    ...
    def test_rakshastra_control_files_requested_writable(self):
        from rakshastra_constants import get_rakshastra_home
