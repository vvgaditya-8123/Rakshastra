...
def _restore_tool_modules():
    original_rakshastra_home = os.environ.get("RAKSHASTRA_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "rakshastra_cli"
        or name.startswith("rakshastra_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_rakshastra_home is None:
            os.environ.pop("RAKSHASTRA_HOME", None)
        else:
            os.environ["RAKSHASTRA_HOME"] = original_rakshastra_home
        _reset_modules(("tools", "rakshastra_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    rakshastra_cli = types.ModuleType("rakshastra_cli")
    rakshastra_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["rakshastra_cli"] = rakshastra_cli
    rakshastra_home = tmp_path / "rakshastra-home"
    os.environ["RAKSHASTRA_HOME"] = str(rakshastra_home)
    sys.modules["rakshastra_cli.config"] = types.SimpleNamespace(
        get_rakshastra_home=lambda: rakshastra_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="rakshastra-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": rakshastra_home / "modal_snapshots.json",
