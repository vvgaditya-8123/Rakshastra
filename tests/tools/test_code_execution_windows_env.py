...
``rakshastra_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``RAKSHASTRA_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # RAKSHASTRA_* handling (#27303): only the operational allowlist passes;
        # every other RAKSHASTRA_* is dropped (the broad prefix was removed).
        "RAKSHASTRA_HOME": "/home/alice/.rakshastra",        # allowlisted → kept
        "RAKSHASTRA_PROFILE": "default",                 # allowlisted → kept
        "RAKSHASTRA_INTERACTIVE": "1",                   # not allowlisted → dropped
        "RAKSHASTRA_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "RAKSHASTRA_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``rakshastra_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``rakshastra_tools.py`` and ``script.py`` writes in
    ...
    def test_file_rpc_stub_uses_utf8(self):
        ...
        from tools.code_execution_tool import generate_rakshastra_tools_module
        stub = generate_rakshastra_tools_module(["terminal"], transport="file")
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        from tools.code_execution_tool import generate_rakshastra_tools_module
        import tempfile, ast
        stub = generate_rakshastra_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_rakshastra_tools_module(["terminal"], transport="uds")
...
# let the child import rakshastra_tools, a user script that printed non-ASCII
