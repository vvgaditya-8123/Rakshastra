"""Shared fixtures for the rakshastra-agent test suite.
...
2. **Isolated RAKSHASTRA_HOME.** RAKSHASTRA_HOME points to a per-test tempdir so
   code reading ``~/.rakshastra/*`` via ``get_rakshastra_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".rakshastra"`` instead of the canonical
   ``get_rakshastra_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No RAKSHASTRA_SESSION_* inheritance** — the agent's current gateway
...
# RAKSHASTRA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_RAKSHASTRA_BEHAVIORAL_VARS = frozenset({
    "RAKSHASTRA_YOLO_MODE",
    "RAKSHASTRA_INTERACTIVE",
    "RAKSHASTRA_QUIET",
    "RAKSHASTRA_TOOL_PROGRESS",
    "RAKSHASTRA_TOOL_PROGRESS_MODE",
    "RAKSHASTRA_MAX_ITERATIONS",
    "RAKSHASTRA_SESSION_PLATFORM",
    "RAKSHASTRA_SESSION_CHAT_ID",
    "RAKSHASTRA_SESSION_CHAT_NAME",
    "RAKSHASTRA_SESSION_THREAD_ID",
    "RAKSHASTRA_SESSION_SOURCE",
    "RAKSHASTRA_SESSION_KEY",
    "RAKSHASTRA_GATEWAY_SESSION",
    "RAKSHASTRA_CRON_SESSION",
    "_RAKSHASTRA_GATEWAY",
    "RAKSHASTRA_PLATFORM",
    "RAKSHASTRA_MODEL",
    "RAKSHASTRA_INFERENCE_MODEL",
    "RAKSHASTRA_INFERENCE_PROVIDER",
    "RAKSHASTRA_TUI_PROVIDER",
    "RAKSHASTRA_MANAGED",
    "RAKSHASTRA_MANAGED_DIR",
    "RAKSHASTRA_DEV",
    "RAKSHASTRA_CONTAINER",
    "RAKSHASTRA_EPHEMERAL_SYSTEM_PROMPT",
    "RAKSHASTRA_TIMEZONE",
    "RAKSHASTRA_REDACT_SECRETS",
    "RAKSHASTRA_BACKGROUND_NOTIFICATIONS",
    "RAKSHASTRA_EXEC_ASK",
    "RAKSHASTRA_HOME_MODE",
    "RAKSHASTRA_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.rakshastra/kanban.db instead of the per-test RAKSHASTRA_HOME.
    "RAKSHASTRA_KANBAN_DB",
    "RAKSHASTRA_KANBAN_BOARD",
    "RAKSHASTRA_KANBAN_HOME",
    "RAKSHASTRA_KANBAN_WORKSPACES_ROOT",
    "RAKSHASTRA_KANBAN_LOGS_ROOT",
    "RAKSHASTRA_KANBAN_TASK",
    "RAKSHASTRA_KANBAN_WORKSPACE",
    "RAKSHASTRA_KANBAN_RUN_ID",
    "RAKSHASTRA_KANBAN_CLAIM_LOCK",
    "RAKSHASTRA_KANBAN_DISPATCH_IN_GATEWAY",
    "RAKSHASTRA_TENANT",
    ...
    "RAKSHASTRA_DASHBOARD_OAUTH_CLIENT_ID",
    "RAKSHASTRA_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and RAKSHASTRA_HOME to per-test tempdirs so code that
    reads ``~/.rakshastra/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral RAKSHASTRA_* vars that could change test semantics.
    for name in _RAKSHASTRA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect RAKSHASTRA_HOME to a per-test tempdir. Code that reads
    #    ``~/.rakshastra/*`` via ``get_rakshastra_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.rakshastra/*`` via
    #    ``Path.home() / ".rakshastra"`` instead of ``get_rakshastra_home()``
    #    is a bug to fix at the callsite.
    fake_rakshastra_home = tmp_path / "rakshastra_test"
    fake_rakshastra_home.mkdir()
    (fake_rakshastra_home / "sessions").mkdir()
    (fake_rakshastra_home / "cron").mkdir()
    (fake_rakshastra_home / "memories").mkdir()
    (fake_rakshastra_home / "skills").mkdir()
    monkeypatch.setenv("RAKSHASTRA_HOME", str(fake_rakshastra_home))
    ...
    #    ~/.rakshastra/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import rakshastra_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_rakshastra_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal rakshastra config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``rakshastra-gateway`` process
...
# correlated with a ``tests/rakshastra_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> rakshastra-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart rakshastra-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting rakshastra/python patterns are also blocked.
    """
    ...
    def _matches_rakshastra_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _RAKSHASTRA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_rakshastra_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch rakshastra-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m rakshastra_cli.main".
                if (
                    "rakshastra" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live rakshastra-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting rakshastra/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `rakshastra update` (or the
        # equivalent `python -m rakshastra_cli.main update`).  These commands
        ...
        if "update" in low and (
            # rakshastra update / rakshastra update --gateway / setsid bash -c ... rakshastra update
            ("rakshastra" in low and "update" in low.split())
            or
            # python -m rakshastra_cli.main update --gateway
            ("rakshastra_cli" in low and "update" in low.split())
            or
            # venv/bin/rakshastra update  (absolute path variant used in tests)
            (".venv/bin/rakshastra" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`rakshastra update` against the real checkout, fetching "
