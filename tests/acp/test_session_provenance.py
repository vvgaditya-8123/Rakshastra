...
def test_root_session_no_compression(db):
    ...
    assert prov["currentRakshastraSessionId"] == "root1"
    assert prov["rootRakshastraSessionId"] == "root1"
    assert prov["parentRakshastraSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_rakshastra_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentRakshastraSessionId"] == "old"
    assert prov["rootRakshastraSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousRakshastraSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootRakshastraSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootRakshastraSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_rakshastra_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousRakshastraSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"rakshastra"}
    assert "sessionProvenance" in meta["rakshastra"]
    assert meta["rakshastra"]["sessionProvenance"]["currentRakshastraSessionId"] == "root1"
