...
Pre-fix bug (issue #33925): ``cli.RakshastraCLI._toggle_yolo`` mutated only
``os.environ["RAKSHASTRA_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``rakshastra --yolo`` (process-start env), ``RAKSHASTRA_YOLO_MODE=1``,
and ``rakshastra config set approvals.mode off`` actually bypassed.
...
read (``session_id``). This avoids the heavy ``RakshastraCLI`` construction
path used in ``test_cli_init.py``, which is incompatible with this test
file's path layout — ``RakshastraCLI.__init__`` imports a lot of optional
...
def _make_stand_in(session_id: str = SESSION_KEY) -> SimpleNamespace:
    ...
    to invoking them on a fully-constructed ``RakshastraCLI`` for the
    behaviour under test, and avoids the brittle prompt_toolkit / config
    stubbing required to instantiate ``RakshastraCLI`` from this test file.
    """
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_enables_session_bypass(self):
        ...
        with patch("cli._cprint"):
            RakshastraCLI._toggle_yolo(stand_in)
    ...
    def test_toggle_yolo_disables_session_bypass_on_second_call(self):
        stand_in = _make_stand_in()
        with patch("cli._cprint"):
            RakshastraCLI._toggle_yolo(stand_in)  # ON
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is True
            RakshastraCLI._toggle_yolo(stand_in)  # OFF
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is False

    def test_toggle_yolo_does_not_mutate_env_var(self):
        """Toggling /yolo must not write ``RAKSHASTRA_YOLO_MODE`` — that path is
        ...
        assert os.environ.get("RAKSHASTRA_YOLO_MODE") is None
    ...
    def test_two_independent_sessions_are_isolated(self):
        ...
        try:
            with patch("cli._cprint"):
                RakshastraCLI._toggle_yolo(cli_a)
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_reflects_toggle(self):
        ...
        assert RakshastraCLI._is_session_yolo_active(stand_in) is False

        with patch("cli._cprint"):
            RakshastraCLI._toggle_yolo(stand_in)

        assert RakshastraCLI._is_session_yolo_active(stand_in) is True
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``rakshastra --yolo`` sets ``RAKSHASTRA_YOLO_MODE`` before tool imports, so
...
class TestToggleYoloEndToEnd:
    ...
    def test_toggle_yolo_bypasses_dangerous_command_check(self):
        ...
        try:
            with patch("cli._cprint"):
                RakshastraCLI._toggle_yolo(stand_in)  # YOLO ON
...
class TestIsSessionYoloActiveAttrSafety:
    ...
    (tests use ``RakshastraCLI.__new__(RakshastraCLI)`` to skip ``__init__``). It must
    ...
    def test_helper_survives_missing_session_id_attr(self):
        ...
        assert RakshastraCLI._is_session_yolo_active(no_attr) is False
...
class TestSessionRotationTransfersYolo:
    ...
    def test_transfer_moves_yolo_to_new_session(self):
        ...
        try:
            ...
            RakshastraCLI._transfer_session_yolo(stand_in, "old-id", "new-id")
    ...
    def test_transfer_is_noop_when_ids_match(self):
        ...
        try:
            ...
            RakshastraCLI._transfer_session_yolo(stand_in, "same-id", "same-id")
    ...
    def test_transfer_handles_empty_inputs_safely(self):
        ...
        # to transfer from "" / to "".
        RakshastraCLI._transfer_session_yolo(stand_in, "", "new")
        RakshastraCLI._transfer_session_yolo(stand_in, "old", "")
