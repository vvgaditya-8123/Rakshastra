...
Override the image with ``RAKSHASTRA_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("RAKSHASTRA_TEST_IMAGE", "rakshastra-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``RAKSHASTRA_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("RAKSHASTRA_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged rakshastra user
# ---------------------------------------------------------------------------
#
# Background: every Rakshastra runtime path inside the container drops to UID
# 10000 (the ``rakshastra`` user) via ``s6-setuidgid rakshastra``. ``docker exec``
...
# fails (PermissionError swallowed) for rakshastra, so a test that ran as root
...
# helpers below run every probe under ``-u rakshastra`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: rakshastra).
