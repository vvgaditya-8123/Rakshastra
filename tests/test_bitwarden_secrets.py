...
binary download are exercised manually by `rakshastra secrets bitwarden
...
def rakshastra_home(tmp_path, monkeypatch):
    """Point Rakshastra at an isolated home directory."""
    home = tmp_path / ".rakshastra"
    home.mkdir()
    monkeypatch.setenv("RAKSHASTRA_HOME", str(home))
    # Some modules cache get_rakshastra_home; clear if needed.
    import rakshastra_constants
    if hasattr(rakshastra_constants, "_RAKSHASTRA_HOME_CACHE"):
        rakshastra_constants._RAKSHASTRA_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(rakshastra_home, monkeypatch):
...
def test_install_bws_happy_path(rakshastra_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(rakshastra_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(rakshastra_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from rakshastra_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under rakshastra_home/cache/."""
    home = tmp_path / ".rakshastra"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
