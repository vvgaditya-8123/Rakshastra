...
The ``rakshastra update`` Python path already clears the conflict with ``git reset``
...
def test_install_sh_clears_unmerged_index_then_stashes(tmp_path: Path) -> None:
    repo = tmp_path / "rakshastra-agent"
    repo.mkdir()
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m rakshastra_cli.main gateway run`` — image name
    ``pythonw``, not ``rakshastra.exe`` — so the ``taskkill /IM rakshastra.exe`` guard
    ...
    # The rakshastra.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM rakshastra.exe' in text
