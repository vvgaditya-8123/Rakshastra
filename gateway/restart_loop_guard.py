...
Defenses 1 and 2 (the ``_RAKSHASTRA_GATEWAY`` guard on ``rakshastra gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.rakshastra.gateway")``,
...
State lives in ``<RAKSHASTRA_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_rakshastra_home() / "gateway" / "restart_loop.json"
