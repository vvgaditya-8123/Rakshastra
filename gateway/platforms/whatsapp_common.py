...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Rakshastra Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to RAKSHASTRA_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/rakshastra), this function
    mirrors the bridge source to a writable RAKSHASTRA_HOME location and returns that
    ...
    # Try RAKSHASTRA_HOME location first
    rakshastra_home = get_rakshastra_home()
    rakshastra_home_bridge = rakshastra_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to RAKSHASTRA_HOME if needed
    if rakshastra_home_bridge.exists():
        return rakshastra_home_bridge

    # Mirror the bridge source to RAKSHASTRA_HOME
    try:
        rakshastra_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            rakshastra_home_bridge,
            dirs_exist_ok=False,
        )
        return rakshastra_home_bridge
    except Exception:
