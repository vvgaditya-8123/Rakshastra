...
There are two ways to add a platform to the Rakshastra gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.rakshastra/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Rakshastra code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `rakshastra gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `rakshastra_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Rakshastra core.
...
And add it to the `rakshastra-gateway` composite:

```python
"rakshastra-gateway": {
    "includes": [..., "rakshastra-your-platform"]
}
```
...
## 12. Status Display (`rakshastra_cli/status.py`)
...
## 13. Gateway Setup Wizard (`rakshastra_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ rakshastra_cli/ toolsets.py \
  --include="*.py" -l | sort -u
