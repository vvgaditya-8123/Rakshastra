...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``rakshastra setup`` display).
    ...
    # built-ins).  Used by ``rakshastra gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `rakshastra`
        # invocation -- including plain `rakshastra chat`, which never touches any
        ...
        # `rakshastra setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``rakshastra setup``/``gateway status``, channel
