#!/usr/bin/env python3
"""Live test harness for Rakshastra Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any rakshastra imports.
ORIGINAL_HOME = os.environ.get("RAKSHASTRA_HOME")
ORIGINAL_AUTH = Path.home() / ".rakshastra" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.rakshastra/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.rakshastra/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="rakshastra_ts_live_"))
    rakshastra_home = home_dir / ".rakshastra"
    rakshastra_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, rakshastra_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".rakshastra" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, rakshastra_home / ".env")
        ...
        load_rakshastra_dotenv(rakshastra_home=str(Path.home() / ".rakshastra"))
    ...
    (rakshastra_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new RAKSHASTRA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "rakshastra_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["RAKSHASTRA_HOME"] = str(home)
...
def main():
    ...
    # Restore original RAKSHASTRA_HOME
    if ORIGINAL_HOME is not None:
        os.environ["RAKSHASTRA_HOME"] = ORIGINAL_HOME
    else:
