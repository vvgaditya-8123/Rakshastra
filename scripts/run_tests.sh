#!/usr/bin/env bash
# Canonical test runner for rakshastra-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.rakshastra/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.rakshastra/rakshastra-agent/venv"; do
...
if [ -f "$HOME/.rakshastra/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.rakshastra"
...
exec env -i \
  ...
  ${RAKSHASTRA_RUN_SLOW_PET_TESTS:+RAKSHASTRA_RUN_SLOW_PET_TESTS="$RAKSHASTRA_RUN_SLOW_PET_TESTS"} \
