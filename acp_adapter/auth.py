"""ACP auth helpers — detect and advertise Rakshastra authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "rakshastra-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Rakshastra runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Rakshastra can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Rakshastra.
    ...
    not have Rakshastra provider credentials configured yet, so Rakshastra always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Rakshastra using the currently configured "
