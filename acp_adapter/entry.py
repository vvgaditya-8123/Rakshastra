"""CLI entry point for the rakshastra-agent ACP adapter.

Loads environment variables from ``~/.rakshastra/.env``, configures logging
...
try:
    ...
    # shadowing Rakshastra's own modules — ``rakshastra acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    rakshastra_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from RAKSHASTRA_HOME (default ``~/.rakshastra``)."""
    from rakshastra_cli.env_loader import load_rakshastra_dotenv

    rakshastra_home = get_rakshastra_home()
    loaded = load_rakshastra_dotenv(rakshastra_home=rakshastra_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", rakshastra_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="rakshastra-acp",
        description="Run Rakshastra Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Rakshastra version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Rakshastra provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.rakshastra/node/ "
...
def _print_version() -> None:
    ...
    print(rakshastra_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import RakshastraACPAgent  # noqa: F401

    print("Rakshastra ACP check OK")


def _run_setup() -> None:
    from rakshastra_cli.main import main as rakshastra_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "rakshastra", "model"]
        rakshastra_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``rakshastra postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting rakshastra-agent ACP adapter")
    ...
    import acp
    from .server import RakshastraACPAgent
