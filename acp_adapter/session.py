"""ACP session manager — maps ACP sessions to Rakshastra AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.rakshastra/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Rakshastra itself is running in WSL.

    Windows ACP clients can launch ``rakshastra acp`` inside WSL while still sending
    ...
    """
    from rakshastra_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Rakshastra from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["rakshastra-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Rakshastra agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Rakshastra AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Rakshastra runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.rakshastra/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``RAKSHASTRA_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_rakshastra_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=rakshastra_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Rakshastra daemon's process cwd.
