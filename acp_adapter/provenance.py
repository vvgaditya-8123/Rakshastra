...
This is an additive Rakshastra extension surfaced under ACP ``_meta.rakshastra`` so
...
compression rotates the internal Rakshastra head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.rakshastra.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_rakshastra_session_id: The live internal Rakshastra DB session id
            (``state.agent.session_id``).
        previous_rakshastra_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"rakshastra": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_rakshastra_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_rakshastra_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_rakshastra_session_id}
    ...
    rotated = bool(
        ...
        and previous_rakshastra_session_id != current_rakshastra_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentRakshastraSessionId": current_rakshastra_session_id,
        "rootRakshastraSessionId": root_id,
        "parentRakshastraSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_rakshastra_session_id:
        provenance["previousRakshastraSessionId"] = previous_rakshastra_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"rakshastra": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"rakshastra": {"sessionProvenance": prov}}
