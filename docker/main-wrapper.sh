...
# shellcheck shell=sh
# /opt/rakshastra/docker/main-wrapper.sh — wraps the container's CMD with
# the same argument-routing logic the pre-s6 entrypoint.sh used. Runs
# as /init's "main program" (Docker CMD) so it inherits stdin/stdout/
# stderr from the container.
#
# Shebang note: /init scrubs env before invoking CMD, so a plain
# `#!/bin/sh` wrapper sees an empty environ and `ENV RAKSHASTRA_HOME=/opt/data`
# from the Dockerfile never reaches `rakshastra`. with-contenv repopulates
# the env from /run/s6/container_environment before exec'ing, which is
# what s6-supervised services use too (see main-rakshastra/run).
#
# Routing:
#   no args                       → exec `rakshastra` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `rakshastra <args>` (subcommand passthrough)
#
# Drop to rakshastra via s6-setuidgid, but skip it when already non-root.
set -e

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid rakshastra "$@"; exec "$@"; }
...
# an arbitrary non-root, non-rakshastra UID, the bootstrap was skipped and the
# baked image dirs (owned by the rakshastra build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u rakshastra)" ]; then
    cat >&2 <<EOF
[rakshastra] ERROR: container started with --user $cur_uid (an arbitrary, non-rakshastra UID) — not supported.
...
# to the rakshastra user's home before dropping privileges so libraries that
...
_rakshastra_orig_cwd="${RAKSHASTRA_ORIG_CWD:-$PWD}"
...
# the user's command so `rakshastra chat` starts in the Docker -w
# directory, not /opt/data.
cd "$_rakshastra_orig_cwd"
...
# Rakshastra subcommand pass-through.
drop rakshastra "$@"
