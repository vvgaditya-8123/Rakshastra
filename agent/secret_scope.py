...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the RAKSHASTRA_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Rakshastra runtime / deployment
    "RAKSHASTRA_HOME", "RAKSHASTRA_PROFILE", "RAKSHASTRA_GATEWAY_LOCK_DIR",
    "RAKSHASTRA_MAX_ITERATIONS", "RAKSHASTRA_MAX_TOKENS", "RAKSHASTRA_API_TIMEOUT",
    "RAKSHASTRA_REDACT_SECRETS", "RAKSHASTRA_NOUS_TIMEOUT_SECONDS",
    "_RAKSHASTRA_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "RAKSHASTRA_KANBAN_DB", "RAKSHASTRA_KANBAN_WORKSPACES_ROOT", "RAKSHASTRA_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "RAKSHASTRA_KANBAN_",
    "RAKSHASTRA_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(rakshastra_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(rakshastra_home) / ".env")
