...
Storage
-------
Bundles live in ``~/.rakshastra/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``RAKSHASTRA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``rakshastra bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under RAKSHASTRA_HOME.

    Honors ``RAKSHASTRA_BUNDLES_DIR`` for tests; falls back to
    ``<RAKSHASTRA_HOME>/skill-bundles``.
    """
    override = os.environ.get("RAKSHASTRA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_rakshastra_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `rakshastra bundles` CLI subcommand.
# ---------------------------------------------------------------------------
