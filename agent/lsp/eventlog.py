...
Grep recipe::
    ...
    tail -f ~/.rakshastra/logs/agent.log | rg 'lsp\\['
"""
...
event_log = logging.getLogger("rakshastra.lint.lsp")
...
def log_server_unavailable(server_id: str, binary_or_pkg: str) -> None:
    ...
    if _announce_once(_announced_unavailable, key):
        _emit(
            ...
            "(install via `rakshastra lsp install <id>` or set lsp.servers.<id>.command)",
        )
    else:
