...
upstream prefix cache warm.  See ``rakshastra-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Rakshastra never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want RAKSHASTRA_HOME persona while keeping
    ...
    # Pointer to the rakshastra-agent skill + docs for user questions about Rakshastra itself.
    stable_parts.append(RAKSHASTRA_AGENT_HELP_GUIDANCE)

    # Modular security, tool execution, and reporting guidelines
    stable_parts.append(SECURITY_GUIDANCE)
    stable_parts.append(SECURITY_REASONING_GUIDANCE)

    # Dynamic active security workflow state and guidance
    if agent.valid_tool_names and "security_workflow" in agent.valid_tool_names:
        try:
            ...
            session_id = getattr(agent, "session_id", "default_session") or "default_session"
            guidance = reasoning.get_guidance(session_id, wf_engine)
            
            wf_block = (
                "# Active Investigation Workflow State\n"
                f"- **Current Active Phase**: {guidance['current_phase']}\n"
                f"- **Recommended Phase Action**: {guidance['recommended_action']}\n"
                f"- **Recommended Skills to consult**: {', '.join(guidance['recommended_skills'])}\n"
                f"- **Workflow Guidelines for Current Phase**: {guidance['guidance_message']}\n"
            )
            stable_parts.append(wf_block)
        except Exception:
            pass

    stable_parts.append(TOOL_GUIDANCE)
    stable_parts.append(REPORTING_GUIDANCE)
    ...
    # RAKSHASTRA_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Rakshastra, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Rakshastra profile the agent is running
    # under so it doesn't conflate ~/.rakshastra/skills/ (default profile) with
    # ~/.rakshastra/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Rakshastra profile: default. Other profiles (if any) live "
            "under ~/.rakshastra/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Rakshastra profile: {active_profile}. This session reads "
            f"and writes ~/.rakshastra/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.rakshastra/skills/, ~/.rakshastra/plugins/, "
            f"~/.rakshastra/cron/, ~/.rakshastra/memories/ — those belong to a "
    ...
    if agent._memory_store:
        if agent._memory_enabled:
            ...
            sec_block = agent._memory_store.format_for_system_prompt("security")
            if sec_block:
                volatile_parts.append(sec_block)
            inc_block = agent._memory_store.format_for_system_prompt("incident")
            if inc_block:
                volatile_parts.append(inc_block)
            ast_block = agent._memory_store.format_for_system_prompt("asset")
            if ast_block:
                volatile_parts.append(ast_block)
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Rakshastra never rebuilds or reinjects parts of it
