...
from rakshastra_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``RAKSHASTRA_KANBAN_TASK`` / ``RAKSHASTRA_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("RAKSHASTRA_KANBAN_TASK") or os.getenv("RAKSHASTRA_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Rakshastra Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``rakshastra_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``RAKSHASTRA_PLATFORM`` or
            ``RAKSHASTRA_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("RAKSHASTRA_PLATFORM")
...
# which becomes the dominant cost of ``rakshastra`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.rakshastra/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``rakshastra`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against RAKSHASTRA_HOME, not cwd
        if not p.is_absolute():
            p = (rakshastra_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.rakshastra/skills/`` first, then external.
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Rakshastra can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    rakshastra = metadata.get("rakshastra") or {}
    if not isinstance(rakshastra, dict):
        rakshastra = {}
    return {
        "fallback_for_toolsets": rakshastra.get("fallback_for_toolsets", []),
        "requires_toolsets": rakshastra.get("requires_toolsets", []),
        "fallback_for_tools": rakshastra.get("fallback_for_tools", []),
        "requires_tools": rakshastra.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    rakshastra = metadata.get("rakshastra")
    if not isinstance(rakshastra, dict):
        return []
    raw = rakshastra.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Rakshastra metadata, VCS, virtualenv/dependency, cache, and skill
