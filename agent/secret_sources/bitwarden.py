...
Rakshastra pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.rakshastra/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<rakshastra_home>/bin/bws`` on
  first use.  Rakshastra pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.rakshastra/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``rakshastra`` invocations don't
  hammer the API.
* Failures NEVER block Rakshastra startup.  Missing binary, no network,
...
# In-process cache so repeated load_rakshastra_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `rakshastra chat -q ...`
...
# <rakshastra_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.rakshastra/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under rakshastra_home/cache/.

    `home_path` is what `load_rakshastra_dotenv()` already resolved; falling back
    to `$RAKSHASTRA_HOME` / `~/.rakshastra` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("RAKSHASTRA_HOME", Path.home() / ".rakshastra"))
...
def _rakshastra_bin_dir() -> Path:
    """Where Rakshastra stores its managed binaries.  Profile-aware."""
    from rakshastra_constants import get_rakshastra_home

    return get_rakshastra_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<rakshastra_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _rakshastra_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``rakshastra secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _rakshastra_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="rakshastra-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "rakshastra-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<rakshastra_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
    ...
    back to ``$RAKSHASTRA_HOME`` / ``~/.rakshastra``.
...
# ---------------------------------------------------------------------------
# Public entry point — called from rakshastra_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_rakshastra_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `rakshastra secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `rakshastra secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `rakshastra secrets bitwarden setup` to install."
        )
        return result
