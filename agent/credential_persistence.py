...
``auth.json``.  They intentionally have no dependency on ``rakshastra_cli.auth`` so
...
# Sources Rakshastra owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Rakshastra-owned OAuth/device-code state)
