...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_RAKSHASTRA_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Rakshastra expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``rakshastra_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_rakshastra_home()`` themselves.
        """
        if "rakshastra_home" not in kwargs:
            from rakshastra_constants import get_rakshastra_home
            kwargs["rakshastra_home"] = str(get_rakshastra_home())
        for provider in self._providers:
            try:
