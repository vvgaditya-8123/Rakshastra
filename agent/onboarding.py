...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Rakshastra starts and finds ``~/.openclaw/``.

    Points users at ``rakshastra claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``rakshastra claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Rakshastra, run "
        "`rakshastra claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `rakshastra claw cleanup` (renames it to ~/.openclaw.pre-migration — "
