...
Plugins built on Rakshastra Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Rakshastra
...
:class:`~rakshastra_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Rakshastra' main config
...
handles every provider, fallback chain, and per-task override Rakshastra
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`rakshastra_cli.plugins.PluginContext`
