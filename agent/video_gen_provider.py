...
as ``kind: backend``) or ``~/.rakshastra/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``rakshastra tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$RAKSHASTRA_HOME/cache/videos/``, creating parents as needed."""
    from rakshastra_constants import get_rakshastra_home

    path = get_rakshastra_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$RAKSHASTRA_HOME/cache/videos/``.
