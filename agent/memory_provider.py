...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `rakshastra backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - rakshastra_home (str): The active RAKSHASTRA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.rakshastra``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "rakshastra").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'rakshastra memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], rakshastra_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'rakshastra memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``rakshastra_home`` is the active RAKSHASTRA_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE RAKSHASTRA_HOME.

        ``rakshastra backup`` only walks RAKSHASTRA_HOME, so any provider state kept
        ...
        of the archive, and ``rakshastra import`` restores them to their original
