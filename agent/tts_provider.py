...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Rakshastra with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.rakshastra/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Rakshastra
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``rakshastra tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``rakshastra setup`` for
