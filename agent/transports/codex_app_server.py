...
projection into Rakshastra' display, approval bridging, transcript projection into
...
"codex_app_server"`. Rakshastra' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Rakshastra secret — gateway
        ...
        spawn_env = rakshastra_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("RAKSHASTRA_KANBAN_TASK"):
            kanban_db = spawn_env.get("RAKSHASTRA_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "RAKSHASTRA_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("RAKSHASTRA_HOME", os.path.expanduser("~/.rakshastra")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "rakshastra",
        client_title: str = "Rakshastra Agent",
