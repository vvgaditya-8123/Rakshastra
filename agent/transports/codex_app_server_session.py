...
Owns one Codex thread per Rakshastra session. Drives `turn/start`, consumes
...
# Rakshastra' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_RAKSHASTRA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Rakshastra/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Rakshastra session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _RAKSHASTRA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("RAKSHASTRA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="rakshastra",
            client_title="Rakshastra Agent",
            client_version=_get_rakshastra_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        into Rakshastra' messages shape.
        ...
        # supports rich content but Rakshastra' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Rakshastra' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own rakshastra-tools callback we
            # auto-accept — the user already approved Rakshastra' tools
            ...
            if server_name == "rakshastra-tools":
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Rakshastra approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Rakshastra returns 'once', 'session', 'always', or 'deny'.
...
def _get_rakshastra_version() -> str:
    """Best-effort Rakshastra version string for codex's userAgent line."""
    try:
