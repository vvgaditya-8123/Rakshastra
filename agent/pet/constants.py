...
row taxonomy is inferred from the atlas shape so Rakshastra can render both legacy
...
# rows (1536x1872), while older Rakshastra/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Rakshastra' activity state names. They are not always identical to the
...
# Legacy Rakshastra/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Rakshastra activity names -> accepted row-name aliases in descending
