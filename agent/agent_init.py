...
from rakshastra_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: rakshastra config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .rakshastra.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / RAKSHASTRA_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.rakshastra/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from rakshastra_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind RAKSHASTRA_DEV_CREDITS.
    ...
    # both live under ~/.rakshastra/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from rakshastra_logging import setup_logging, setup_verbose_logging
    setup_logging(rakshastra_home=_ra()._rakshastra_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that rakshastra_logging.setup_logging() does not
        ...
        # handler level inside rakshastra_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from rakshastra_cli.auth import build_minimax_oauth_token_provider
            ...
            agent._client_kwargs = {}
            if not agent.quiet_mode:
                print(f"🛡️ Rakshastra Security OS initialized with model: {agent.model} (Anthropic native)")
        ...
        try:
            from rakshastra_cli.config import load_config as _load_br_cfg
        ...
        if not agent.quiet_mode:
            ...
            print(f"🛡️ Rakshastra Security OS initialized with model: {agent.model} (AWS Bedrock, {agent._bedrock_region}{_gr_label})")
    else:
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `rakshastra model` to "
                        "select a provider, or run `rakshastra setup` for first-time "
                        "configuration."
                    )
        ...
        try:
            ...
            if not agent.quiet_mode:
                print(f"🛡️ Rakshastra Security OS initialized with model: {agent.model}")
                if base_url:
    ...
    # worker (kanban_show tool is present iff RAKSHASTRA_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["RAKSHASTRA_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.rakshastra/sessions/ alongside gateway sessions
    rakshastra_home = get_rakshastra_home()
    agent.logs_dir = rakshastra_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.rakshastra/sessions/session_{sid}.json)
    ...
    try:
        from rakshastra_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from rakshastra_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "rakshastra"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Rakshastra' built-in
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from rakshastra_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from rakshastra_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Rakshastra Agent.  Choose a model with at least "
