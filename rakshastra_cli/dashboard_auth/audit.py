...
Profile-aware location: ``$RAKSHASTRA_HOME/logs/dashboard-auth.log``.
...
from ``rakshastra_constants`` or other rakshastra_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$RAKSHASTRA_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``rakshastra_constants.get_rakshastra_home`` semantics: env var wins,
    else ``~/.rakshastra``. A local copy avoids an import cycle with the
    middleware which lives below ``rakshastra_cli``.
    """
    home = os.environ.get("RAKSHASTRA_HOME") or str(Path.home() / ".rakshastra")
