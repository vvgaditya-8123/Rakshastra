"""``rakshastra dashboard`` / ``rakshastra serve`` subcommand parsers.
...
``rakshastra_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `rakshastra dashboard` / `rakshastra serve` cmdlines and SIGTERM them directly —
    # the same path `rakshastra update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Rakshastra web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Rakshastra web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Rakshastra Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Rakshastra desktop app shells (<= 0.15.x) spawn the
    # backend as `rakshastra dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Rakshastra couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Rakshastra Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Rakshastra backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Rakshastra backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # `rakshastra dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.rakshastra/.env.
    # Nested subparser so bare `rakshastra dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "RAKSHASTRA_DASHBOARD_OAUTH_CLIENT_ID into ~/.rakshastra/.env, and prints "
            "how to engage the login gate. Requires being logged in (rakshastra setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://rakshastra.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via RAKSHASTRA_DASHBOARD_PORTAL_URL. Mainly for "
