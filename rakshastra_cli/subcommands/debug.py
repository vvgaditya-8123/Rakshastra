"""``rakshastra debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Rakshastra Agent. Use 'rakshastra debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    rakshastra debug share              Upload debug report (asks for confirmation)
    rakshastra debug share --yes        Skip confirmation (for scripts/CI)
    rakshastra debug share --lines 500  Include more log lines
    rakshastra debug share --expire 30  Keep paste for 30 days
    rakshastra debug share --local      Print report locally (no upload)
    rakshastra debug share --no-redact  Disable upload-time secret redaction
    rakshastra debug share --nous       Upload to Nous-internal storage (private)
    rakshastra debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'rakshastra debug share'",
    )
