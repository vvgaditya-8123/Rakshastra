...
``<root>`` is the **shared Rakshastra root** (the parent of any active
...
``rakshastra -p <profile>`` joins the same board as the dispatcher that
...
the profile's Rakshastra home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``RAKSHASTRA_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``RAKSHASTRA_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``rakshastra kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.rakshastra``. In Docker / custom
deployments where ``RAKSHASTRA_HOME`` points outside ``~/.rakshastra`` (e.g.
``/opt/rakshastra``), ``<root>`` is ``RAKSHASTRA_HOME``. Legacy env-var
overrides still work:

* ``RAKSHASTRA_KANBAN_DB`` — pin the database file path directly.
* ``RAKSHASTRA_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``RAKSHASTRA_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``RAKSHASTRA_KANBAN_DB``,
``RAKSHASTRA_KANBAN_WORKSPACES_ROOT``, and ``RAKSHASTRA_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/rakshastra-kanban-v1-spec.pdf`` for the full
...
from rakshastra_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active RAKSHASTRA_HOME so dispatcher- and
...
# ``RAKSHASTRA_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``RAKSHASTRA_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("RAKSHASTRA_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``RAKSHASTRA_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("RAKSHASTRA_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``RAKSHASTRA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "RAKSHASTRA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "rakshastra_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``rakshastra-agent``
...
def kanban_home() -> Path:
    """Return the shared Rakshastra root that anchors the kanban board.

    Resolution order:

    1. ``RAKSHASTRA_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_rakshastra_root()``, which already returns ``<root>``
       when ``RAKSHASTRA_HOME`` is ``<root>/profiles/<name>``, and returns
       ``RAKSHASTRA_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``RAKSHASTRA_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("RAKSHASTRA_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from rakshastra_constants import get_default_rakshastra_root
    return get_default_rakshastra_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``rakshastra kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``RAKSHASTRA_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``rakshastra kanban boards
    ...
    env = os.environ.get("RAKSHASTRA_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``rakshastra kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``RAKSHASTRA_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("RAKSHASTRA_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``RAKSHASTRA_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("RAKSHASTRA_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``RAKSHASTRA_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("RAKSHASTRA_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``rakshastra kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``RAKSHASTRA_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (rakshastra_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``RAKSHASTRA_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("RAKSHASTRA_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``rakshastra gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Rakshastra treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``RAKSHASTRA_KANBAN_DB`` env → ``RAKSHASTRA_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `rakshastra kanban list` mid-integrity-probe) block
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from rakshastra_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See rakshastra_state._WAL_INCOMPAT_MARKERS for detection logic.
                from rakshastra_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``rakshastra kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``RAKSHASTRA_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from rakshastra_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``rakshastra --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        try:
            from rakshastra_cli import projects_db as _pdb
    ...
    # `rakshastra --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``rakshastra kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `rakshastra kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``rakshastra kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``rakshastra kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``rakshastra kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``rakshastra kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``RAKSHASTRA_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Rakshastra' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Rakshastra-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``rakshastra kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Rakshastra checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Rakshastra treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Rakshastra creates/reuses that linked worktree. With
      no ``workspace_path``, Rakshastra anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Rakshastra uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``RAKSHASTRA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Rakshastra
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``rakshastra kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``rakshastra kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Rakshastra profile.
    ...
    if not rows:
        return False
    try:
        from rakshastra_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from rakshastra_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Rakshastra profile.
        # `_default_spawn` invokes ``rakshastra -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Rakshastra
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `rakshastra kanban tail` — without
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from rakshastra_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_rakshastra_argv() -> list[str]:
    """Return the interpreter-bound Rakshastra CLI invocation."""
    # ``rakshastra_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``rakshastra`` package — there is no
    # ``rakshastra`` package to import.
    return [sys.executable, "-m", "rakshastra_cli.main"]


def _absolute_rakshastra_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Rakshastra shim."""
...
def _rakshastra_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Rakshastra executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_rakshastra_argv()
    return [_absolute_rakshastra_path(path)]


def _resolve_rakshastra_argv() -> list[str]:
    """Resolve the ``rakshastra`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$RAKSHASTRA_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("rakshastra")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\rakshastra.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m rakshastra_cli.main`` — fallback for setups where
       Rakshastra is launched from a venv and the ``rakshastra`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_rakshastra_bin`` for the same reason. Kept
    local (not imported from gateway) because ``rakshastra_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("RAKSHASTRA_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _rakshastra_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _rakshastra_path_argv(resolved_env_bin)
        return _module_rakshastra_argv()

    rakshastra_bin = _safe_which_no_cwd("rakshastra") if _IS_WINDOWS else shutil.which("rakshastra")
    if rakshastra_bin:
        return _rakshastra_path_argv(rakshastra_bin)
    return _module_rakshastra_argv()
...
def _resolve_worker_cli_toolsets(rakshastra_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``RAKSHASTRA_KANBAN_TASK`` is set.
    """
    if not rakshastra_home:
        return None
    try:
        from rakshastra_constants import reset_rakshastra_home_override, set_rakshastra_home_override
        from rakshastra_cli.config import load_config
        from rakshastra_cli.tools_config import _get_platform_tools

        token = set_rakshastra_home_override(rakshastra_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for RAKSHASTRA_HOME=%r (%s)",
            rakshastra_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``rakshastra -p <profile> chat -q ...`` subprocess.
    ...
    ``RAKSHASTRA_KANBAN_DB`` / ``RAKSHASTRA_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject RAKSHASTRA_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `rakshastra -p <name>` the
    # _apply_profile_override() runs *before* rakshastra_constants is imported.
    # If RAKSHASTRA_HOME is absent from the child's env, get_rakshastra_home() falls
    # back to Path.home() / ".rakshastra" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from rakshastra_cli.profiles import resolve_profile_env
    try:
        env["RAKSHASTRA_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via RAKSHASTRA_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # RAKSHASTRA_HOME never had profiles created.
        pass
    if task.tenant:
        env["RAKSHASTRA_TENANT"] = task.tenant
    env["RAKSHASTRA_KANBAN_TASK"] = task.id
    env["RAKSHASTRA_KANBAN_WORKSPACE"] = workspace
    ...
    if task.branch_name:
        env["RAKSHASTRA_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["RAKSHASTRA_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["RAKSHASTRA_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["RAKSHASTRA_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["RAKSHASTRA_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`rakshastra -p <name>`
    # rewrites RAKSHASTRA_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_rakshastra_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["RAKSHASTRA_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["RAKSHASTRA_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["RAKSHASTRA_KANBAN_BOARD"] = resolved_board
    # RAKSHASTRA_PROFILE is the author the kanban_comment tool defaults to.
    # `rakshastra -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["RAKSHASTRA_PROFILE"] = profile_arg

    cmd = [
        *_resolve_rakshastra_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped RAKSHASTRA_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("RAKSHASTRA_HOME"))
    if worker_toolsets:
    ...
    # `rakshastra kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`rakshastra` executable not found on PATH. "
            "Install Rakshastra Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``rakshastra kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled RAKSHASTRA_PROFILE values like "rakshastra-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Rakshastra root exists

    Reads profile paths directly so this module has no import dependency on
    ``rakshastra_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from rakshastra_constants import get_default_rakshastra_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``rakshastra kanban assignees`` for the terminal.
