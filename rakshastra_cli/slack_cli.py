"""``rakshastra slack ...`` CLI subcommands.

Today only ``rakshastra slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ rakshastra slack manifest > slack-manifest.json
    # or:
    $ rakshastra slack manifest --write
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Rakshastra. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Rakshastra deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from rakshastra_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Rakshastra in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Rakshastra agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``rakshastra_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$RAKSHASTRA_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Rakshastra")
    ...
    """
    name = getattr(args, "name", None) or "Rakshastra"
    description = getattr(args, "description", None) or "Your Rakshastra agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("RAKSHASTRA_HOME") or str(Path.home() / ".rakshastra")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Rakshastra app\n"
