"""Profile distributions — shareable, packaged Rakshastra profiles via git.

A distribution is a Rakshastra profile published as a git repository (or
...
* ``rakshastra profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``rakshastra skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``rakshastra profile``, not a parallel tree):

    rakshastra profile install <source> [--name N] [--alias] [--force] [--yes]
    rakshastra profile update  <name>  [--force-config] [--yes]
    rakshastra profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "rakshastra_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".rakshastra_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "rakshastra-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            rakshastra_requires=str(data.get("rakshastra_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.rakshastra_requires:
            out["rakshastra_requires"] = self.rakshastra_requires
        if self.author:
...
def check_rakshastra_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Rakshastra {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Rakshastra distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``rakshastra profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Rakshastra profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Rakshastra distribution."
        )

    # Version check up-front so we fail fast
    check_rakshastra_requires(manifest.rakshastra_requires, rakshastra_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.rakshastra).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="rakshastra_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `rakshastra profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`rakshastra profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="rakshastra_dist_update_") as tmp:
        plan = plan_install(
