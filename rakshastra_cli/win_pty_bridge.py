"""Windows ConPTY bridge for the `rakshastra dashboard` chat tab.

Drop-in counterpart to ``rakshastra_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``rakshastra_cli.web_server`` consumes: ``spawn``, ``read``,
