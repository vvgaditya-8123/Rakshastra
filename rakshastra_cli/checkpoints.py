"""`rakshastra checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.rakshastra/checkpoints/``.  Actions:

    rakshastra checkpoints               # same as `status`
    rakshastra checkpoints status        # total size, project count, breakdown
    rakshastra checkpoints list          # per-project checkpoint counts + workdir
    rakshastra checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    rakshastra checkpoints clear [-f]    # nuke the entire base (asks first)
    rakshastra checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    rakshastra checkpoints
    rakshastra checkpoints prune --retention-days 3 --max-size-mb 200
    rakshastra checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: rakshastra checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``rakshastra checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `rakshastra checkpoints` → status
