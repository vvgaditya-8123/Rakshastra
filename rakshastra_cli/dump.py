"""
Dump command for rakshastra CLI.

Outputs a compact, plain-text summary of the user's Rakshastra setup
...
from rakshastra_cli.config import get_rakshastra_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.rakshastra_build_sha`` by the Dockerfile's
    ``RAKSHASTRA_GIT_SHA`` build-arg (see ``rakshastra_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``rakshastra dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from rakshastra_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(rakshastra_home: Path) -> int:
...
def _cron_summary(rakshastra_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = rakshastra_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    load_rakshastra_dotenv(
        rakshastra_home=env_path.parent,
    ...
    lines.append(f"rakshastra_home:      {display_rakshastra_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `rakshastra auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `rakshastra auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["rakshastra-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(rakshastra_home)}")
    lines.append(f"  skills:             {_count_skills(rakshastra_home)}")
