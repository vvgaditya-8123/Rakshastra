...
manual BotFather token copy-paste. Rakshastra talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.rakshastra-agent.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "RakshastraSetupBot"

DEFAULT_BOT_NAME = "Rakshastra Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``rakshastra_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``rakshastra_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"rakshastra_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Rakshastra Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Rakshastra Telegram onboarding service.")
