"""rakshastra hooks — inspect and manage shell-script hooks.

Usage::

    rakshastra hooks list
    rakshastra hooks test <event> [--for-tool X] [--payload-file F]
    rakshastra hooks revoke <command>
    rakshastra hooks doctor

Consent records live under ``~/.rakshastra/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.rakshastra/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``rakshastra hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: rakshastra hooks {list|test|revoke|doctor}")
        print("Run 'rakshastra hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.rakshastra/config.yaml.")
        print("See `rakshastra hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `rakshastra hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `rakshastra hooks test` and `rakshastra hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Rakshastra wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `rakshastra hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `rakshastra hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `rakshastra hooks doctor`.")
