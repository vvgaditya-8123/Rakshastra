...
the tool registry or launching Rakshastra (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``rakshastra-cli`` to the non-existent ``rakshastra``. ``resolve_toolset('rakshastra')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``rakshastra-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``rakshastra`` instead of ``rakshastra-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        for name in names:
            ...
            if is_valid_toolset(name):
                valid_count += 1
                continue
            suggestion = f"rakshastra-{platform}"
            hint = (
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `rakshastra tools` to reconfigure."
        )
    return warnings
