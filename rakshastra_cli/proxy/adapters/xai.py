...
# xAI's public API is OpenAI-compatible for the endpoints Rakshastra commonly
# uses. The Responses endpoint is included because Rakshastra' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Rakshastra-managed OAuth credentials."""

    auth_hint = "rakshastra auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`rakshastra auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`rakshastra auth reset xai-oauth` or re-authenticate with "
                    "`rakshastra auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `rakshastra auth add xai-oauth --type oauth`."
            )
