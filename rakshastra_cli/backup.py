"""
Backup and import commands for rakshastra CLI.

`rakshastra backup` creates a zip archive of the entire ~/.rakshastra/ directory
(excluding the rakshastra-agent repo and transient files).

`rakshastra import` restores from a backup zip, overlaying onto the current
RAKSHASTRA_HOME root.
"""
...
from rakshastra_constants import get_default_rakshastra_root, get_rakshastra_home, display_rakshastra_home
...
# ``rakshastra-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/rakshastra-agent/``
...
# RAKSHASTRA_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "rakshastra-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under RAKSHASTRA_HOME) —
...
# File names that ``rakshastra import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE RAKSHASTRA_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside RAKSHASTRA_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to rakshastra root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``rakshastra-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/rakshastra-agent/`` — must be preserved.
        if part == "rakshastra-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside RAKSHASTRA_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Rakshastra home directory."""
    rakshastra_root = get_default_rakshastra_root()

    if not rakshastra_root.is_dir():
        print(f"Error: Rakshastra home directory not found at {rakshastra_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"rakshastra-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"rakshastra-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_rakshastra_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(rakshastra_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(rakshastra_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``rakshastra-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "rakshastra-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(rakshastra_root)
    ...
    # outside RAKSHASTRA_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_rakshastra_home()}."
        )
    ...
    print(f"\nRestore with: rakshastra import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Rakshastra backup.
    ...
    # Look for telltale files that a rakshastra home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Rakshastra backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.rakshastra/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a rakshastra dir name
        if prefix in {".rakshastra", "rakshastra"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_rakshastra_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (rakshastra_root / "config.yaml").exists()
        has_env = (rakshastra_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Rakshastra configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under RAKSHASTRA_HOME.
            ...
            target = rakshastra_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(rakshastra_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_rakshastra_home()}."
            )
        ...
        profiles_dir = rakshastra_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # rakshastra_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: rakshastra profile list  (after installing rakshastra)")

        # Guidance
        print()
        if not (rakshastra_root / "rakshastra-agent").is_dir():
            print("Note: The rakshastra-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: rakshastra update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  rakshastra -p {pname} gateway install")

        print("Done. Your Rakshastra configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and rakshastra backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to RAKSHASTRA_HOME).
...
# — `rakshastra update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $RAKSHASTRA_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside RAKSHASTRA_HOME and the entry is silently
...
def _quick_snapshot_root(rakshastra_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(rakshastra_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside RAKSHASTRA_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``rakshastra update``.
    ...
    Args:
        ...
        rakshastra_home: Override for the Rakshastra home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(rakshastra_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for rakshastra backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_rakshastra_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, rakshastra_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``rakshastra_root`` to ``out_path``.
...
def _pre_update_backup_dir(rakshastra_home: Optional[Path] = None) -> Path:
    home = rakshastra_home or get_rakshastra_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of RAKSHASTRA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<RAKSHASTRA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``rakshastra update``) should continue even if the backup fails.
    """
    rakshastra_root = rakshastra_home or get_default_rakshastra_root()
    if not rakshastra_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(rakshastra_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, rakshastra_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `rakshastra claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of RAKSHASTRA_HOME under ``backups/`` before a
    ``rakshastra claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``rakshastra import <archive>``.  Writes to
    ``<RAKSHASTRA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `rakshastra import` and the
