...
Pure logic: walks a Rakshastra config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `rakshastra doctor` and a future `rakshastra migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Rakshastra config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Rakshastra config and return retirement issues.
