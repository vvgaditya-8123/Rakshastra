"""
Rakshastra Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.rakshastra/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Rakshastra PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing rakshastra-agent or rakshastra PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Rakshastra Agent" comment and following line
                if '# Rakshastra Agent' in line or '# rakshastra-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('rakshastra' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing rakshastra
                if 'rakshastra' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the rakshastra wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "rakshastra",
        Path("/usr/local/bin/rakshastra"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains rakshastra_cli reference)
                content = wrapper.read_text()
                if 'rakshastra_cli' in content or 'rakshastra-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(rakshastra_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``rakshastra`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Rakshastra home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (rakshastra_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``rakshastra gateway run`` processes
    ...
    try:
        from rakshastra_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from rakshastra_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``RAKSHASTRA_HOME`` and ``RAKSHASTRA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\rakshastra\git\cmd``, ``%LOCALAPPDATA%\rakshastra\git\bin``,
#      ``%LOCALAPPDATA%\rakshastra\git\usr\bin``, ``%LOCALAPPDATA%\rakshastra\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\rakshastra\git\`` and Node to
#      ``%LOCALAPPDATA%\rakshastra\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``rakshastra dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\rakshastra\gateway-service\`` and sometimes
...
def _rakshastra_path_markers(rakshastra_home: Path) -> list[str]:
    """Path-entry substrings that identify Rakshastra-owned User-PATH entries."""
    root = str(rakshastra_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare rakshastra-agent install dir.
    markers = [root + "\\rakshastra-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if RAKSHASTRA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "rakshastra".  We don't want to catch
    # unrelated entries like "crakshastra-foo" or "ephermeral", so we look for
    # backslash-rakshastra as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(rakshastra_home: Path) -> list[str]:
    """Strip Rakshastra-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _rakshastra_path_markers(rakshastra_home)
            kept: list[str] = []
...
def remove_rakshastra_env_vars_windows() -> list[str]:
    """Delete RAKSHASTRA_HOME and RAKSHASTRA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("RAKSHASTRA_HOME", "RAKSHASTRA_GIT_BASH_PATH"):
                try:
...
def _is_default_rakshastra_home(rakshastra_home: Path) -> bool:
    """Return True when ``rakshastra_home`` points at the default (non-profile) root."""
    try:
        from rakshastra_constants import get_default_rakshastra_root
        return rakshastra_home.resolve() == get_default_rakshastra_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its RAKSHASTRA_HOME directory.

    We shell out to ``rakshastra -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current RAKSHASTRA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m rakshastra_cli.main` so we don't depend on a `rakshastra`
    #    wrapper that may be half-removed mid-uninstall.
    rakshastra_invocation = [_sys.executable, "-m", "rakshastra_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                rakshastra_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's RAKSHASTRA_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``rakshastra uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$RAKSHASTRA_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from rakshastra_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(rakshastra_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Rakshastra Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Rakshastra Chat GUI installation was found.")
        print(f"  Checked: {rakshastra_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Rakshastra agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(rakshastra_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Rakshastra agent at {rakshastra_home / 'rakshastra-agent'}")
        print(f"  • Your config, sessions, and secrets under {rakshastra_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Rakshastra agent is still installed. Run 'rakshastra' to use the CLI,")
    print("or 'rakshastra uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.rakshastra/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.rakshastra/ for future reinstall
    """
    project_root = get_project_root()
    rakshastra_home = get_rakshastra_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie RAKSHASTRA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_rakshastra_home(rakshastra_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.rakshastra data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Rakshastra Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {rakshastra_home / 'config.yaml'}")
    print(f"  Secrets: {rakshastra_home / '.env'}")
    print(f"  Data:    {rakshastra_home / 'cron/'}, {rakshastra_home / 'sessions/'}, {rakshastra_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their RAKSHASTRA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Rakshastra data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Rakshastra code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``rakshastra`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$RAKSHASTRA_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in rakshastra_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\rakshastra\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(rakshastra_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Rakshastra-owned PATH entries in User environment")

        log_info("Removing RAKSHASTRA_HOME / RAKSHASTRA_GIT_BASH_PATH User env vars...")
        removed_env = remove_rakshastra_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Rakshastra-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing rakshastra command...")
    ...
    #     (only when they still point into this Rakshastra home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Rakshastra-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(rakshastra_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Rakshastra-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(rakshastra_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE RAKSHASTRA_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from rakshastra_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(rakshastra_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.rakshastra/, just remove the rakshastra-agent subdir
            if rakshastra_home in project_root.parents or project_root.parent == rakshastra_home:
    ...
    #     under RAKSHASTRA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(rakshastra_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(rakshastra_home)
    ...
    # 5. Optionally remove ~/.rakshastra/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile RAKSHASTRA_HOME dirs live under
        ...
        try:
            if rakshastra_home.exists():
                shutil.rmtree(rakshastra_home)
                log_success(f"Removed {rakshastra_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {rakshastra_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {rakshastra_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {rakshastra_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://rakshastra-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://rakshastra-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Rakshastra Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m rakshastra_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import rakshastra_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``rakshastra_constants`` + ``rakshastra_cli.colors``
    (and lazily ``rakshastra_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m rakshastra_cli.uninstall")
    parser.add_argument(
        "--mode",
