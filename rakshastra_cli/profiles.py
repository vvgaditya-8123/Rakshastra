"""
Profile management for multiple isolated Rakshastra instances.

Each profile is a fully independent RAKSHASTRA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.rakshastra/profiles/<name>/`` by default.

The "default" profile is ``~/.rakshastra`` itself — backward compatible,
zero migration needed.

Usage::

    rakshastra profile create coder          # fresh profile + bundled skills
    rakshastra profile create coder --clone  # also copy config, .env, SOUL.md, skills
    rakshastra profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    rakshastra -p coder chat                 # or via flag
    rakshastra profile use coder             # set as sticky default
    rakshastra profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See rakshastra_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.rakshastra``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   rakshastra-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `rakshastra backup` archives
...
# Marker file written by `rakshastra profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `rakshastra
...
# `rakshastra skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Rakshastra infrastructure directories that only the default profile
         (``~/.rakshastra``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_rakshastra_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.rakshastra) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "rakshastra-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "rakshastra", "default", "test", "tmp", "root", "sudo",
})

# Rakshastra subcommands that cannot be used as profile names/aliases
_RAKSHASTRA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the rakshastra root, NOT to the current RAKSHASTRA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where RAKSHASTRA_HOME points outside
    ``~/.rakshastra``, profiles live under ``RAKSHASTRA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_rakshastra_home() / "profiles"


def _get_default_rakshastra_home() -> Path:
    """Return the default (pre-profile) RAKSHASTRA_HOME path.

    In standard deployments this is ``~/.rakshastra``.
    In Docker/custom deployments where RAKSHASTRA_HOME is outside ``~/.rakshastra``
    (e.g. ``/opt/data``), returns RAKSHASTRA_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_rakshastra_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Rakshastra installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its RAKSHASTRA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_rakshastra_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, rakshastra subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _RAKSHASTRA_SUBCOMMANDS:
        return f"'{canon}' conflicts with a rakshastra subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            rakshastra_exe = shutil.which("rakshastra") or "rakshastra"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(rakshastra_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Rakshastra. If we
    ...
    try:
        ...
        # not be migrated. The next `rakshastra doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``rakshastra -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``rakshastra profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``rakshastra -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``rakshastra profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``RAKSHASTRA_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Rakshastra config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_rakshastra_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, rakshastra_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own RAKSHASTRA_HOME.
    ...
    The returned ``rakshastra_home`` is the path to pass to
    ``set_rakshastra_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_rakshastra_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``rakshastra update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.rakshastra)."
        )
    ...
    if clone_from is not None or clone_all or clone_config:
        if clone_from is None:
            ...
            source_dir = get_rakshastra_home()
        else:
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.rakshastra/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `rakshastra -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Rakshastra profile.\n"
    ...
    if not soul_path.exists():
        try:
            from rakshastra_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `rakshastra update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`rakshastra profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Rakshastra (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `rakshastra profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `rakshastra -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches RAKSHASTRA_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``rakshastra profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "RAKSHASTRA_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_rakshastra_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.rakshastra).\n"
            "To remove everything, use: rakshastra uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``RAKSHASTRA_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from rakshastra_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set RAKSHASTRA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("RAKSHASTRA_HOME")
    try:
        os.environ["RAKSHASTRA_HOME"] = str(profile_dir)
        from rakshastra_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["RAKSHASTRA_HOME"] = old_home
        elif "RAKSHASTRA_HOME" in os.environ:
            del os.environ["RAKSHASTRA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.rakshastra/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: rakshastra profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from RAKSHASTRA_HOME.

    Returns ``"default"`` if RAKSHASTRA_HOME is not set or points to ``~/.rakshastra``.
    Returns the profile name if RAKSHASTRA_HOME points into ``~/.rakshastra/profiles/<name>``.
    Returns ``"custom"`` if RAKSHASTRA_HOME is set to an unrecognized path.
    """
    from rakshastra_constants import get_rakshastra_home
    rakshastra_home = get_rakshastra_home()
    resolved = rakshastra_home.resolve()

    default_resolved = _get_default_rakshastra_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.rakshastra itself — its parent is ~/ and its
        # directory name is ".rakshastra", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: rakshastra profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.rakshastra itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.rakshastra). "
            "Specify a different name: rakshastra profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="rakshastra_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"rakshastra_{old_name}"
    legacy_old_host = f"rakshastra.{old_name}"
    new_host = f"rakshastra_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_rakshastra_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("rakshastra_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a RAKSHASTRA_HOME path string.

    Called early in the CLI entry point, before any rakshastra modules
    are imported, to set the RAKSHASTRA_HOME environment variable.
    """
