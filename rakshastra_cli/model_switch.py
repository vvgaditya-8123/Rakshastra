...
- ``rakshastra_cli.providers``        -- canonical provider identity + overlays
- ``rakshastra_cli.model_normalize``  -- per-provider name formatting
...
_RAKSHASTRA_MODEL_WARNING = (
    "Rakshastra 3 & 4 chat models are NOT agentic and are not designed "
    "for use with Rakshastra Cyber Defense Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the Rakshastra 3 / Rakshastra 4 chat families.
# The previous substring check (`"rakshastra" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``rakshastra-brain:qwen3-14b-ctx16k`` that just
# happen to carry "rakshastra" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Rakshastra-3-Llama-3.1-70B, rakshastra-4-405b, openrouter/rakshastra3:70b
# Negative examples it must NOT match:
#   rakshastra-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_RAKSHASTRA_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])rakshastra[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_rakshastra_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Rakshastra 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_RAKSHASTRA_NON_AGENTIC_RE.search(model_name))


def _check_rakshastra_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Rakshastra 3/4 chat model."""
    if is_nous_rakshastra_non_agentic(model_name):
        return _RAKSHASTRA_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``rakshastra config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from rakshastra_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'rakshastra model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'rakshastra doctor' — config issues detected:"
        ...
        from rakshastra_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from rakshastra_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from rakshastra_cli.config import load_config
            from rakshastra_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from rakshastra_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in rakshastra_cli.runtime_provider.resolve_runtime_provider;
    ...
    rakshastra_warn = _check_rakshastra_model_warning(new_model)
    if rakshastra_warn:
        warnings.append(rakshastra_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from rakshastra_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from rakshastra_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from rakshastra_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by rakshastra provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://rakshastra-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Rakshastra release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from rakshastra_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from rakshastra_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Rakshastra-mapped providers ---
    from rakshastra_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from rakshastra_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for rakshastra_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(rakshastra_id)
        if (
            _alias_target
            and _alias_target != rakshastra_id
        ...
        pconfig = PROVIDER_REGISTRY.get(rakshastra_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (RAKSHASTRA_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from rakshastra_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(rakshastra_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `rakshastra model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(rakshastra_id)
        if not model_ids:
            model_ids = curated.get(rakshastra_id, [])
            if rakshastra_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(rakshastra_id, model_ids)
        total = len(model_ids)
        if rakshastra_id in _UNCAPPED_PICKER_PROVIDERS:
    ...
    # --- 2. Check Rakshastra-only providers (nous, openai-codex, copilot, opencode-go) ---
    from rakshastra_cli.providers import RAKSHASTRA_OVERLAYS
    from rakshastra_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Rakshastra provider ID.
    # RAKSHASTRA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Rakshastra IDs ("copilot").
    _mdev_to_rakshastra = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in RAKSHASTRA_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Rakshastra slug — e.g. "github-copilot" → "copilot"
        rakshastra_slug = _mdev_to_rakshastra.get(pid, pid)
        if rakshastra_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(rakshastra_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, rakshastra_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or rakshastra_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(rakshastra_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", rakshastra_slug, exc)
        ...
        # configured.
        if not has_creds and rakshastra_slug == "anthropic":
            try:
                ...
                if (rakshastra_creds and rakshastra_creds.get("accessToken")) or \
        ...
        if rakshastra_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(rakshastra_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(rakshastra_slug)
                model_ids = _ids if _ids else (curated.get(rakshastra_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(rakshastra_slug, []) or curated.get(pid, [])
        elif rakshastra_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `rakshastra model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from rakshastra_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(rakshastra_slug, []) or curated.get(pid, [])
                if rakshastra_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(rakshastra_slug, model_ids)
        total = len(model_ids)
        if rakshastra_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids

        results.append({
            "slug": rakshastra_slug,
            "name": get_label(rakshastra_slug),
            "is_current": rakshastra_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "rakshastra",
        ...
        seen_slugs.add(rakshastra_slug.lower())
        _record_builtin_endpoint(rakshastra_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or RAKSHASTRA_OVERLAYS (keeps /model in sync
    # with `rakshastra model`).
    try:
        from rakshastra_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Rakshastra's canonical write key (matches
            ...
            # Rakshastra writes ``models:`` as a dict keyed by model id
            # (see rakshastra_cli/main.py::_save_custom_provider); older
            ...
            if should_probe:
                try:
                    from rakshastra_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Rakshastra's own writer uses; a hyphen variant is accepted
            ...
            # active model. Rakshastra's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, rakshastra_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from rakshastra_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`rakshastra_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from rakshastra_cli.models import fetch_openrouter_models
