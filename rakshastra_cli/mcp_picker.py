"""MCP picker — interactive `rakshastra mcp picker` (also the default `rakshastra mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``rakshastra mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `rakshastra plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from rakshastra_cli.cli_output import prompt_yes_no
from rakshastra_cli.curses_ui import curses_single_select
from rakshastra_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Rakshastra session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from rakshastra_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `rakshastra mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: rakshastra mcp install <name>    Picker: rakshastra mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Rakshastra is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Rakshastra — run `rakshastra update` "
...
def show_catalog() -> None:
    """`rakshastra mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`rakshastra mcp picker` (and default `rakshastra mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`rakshastra mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `rakshastra mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
