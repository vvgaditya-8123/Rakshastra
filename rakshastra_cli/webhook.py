"""rakshastra webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    rakshastra webhook subscribe <name> [options]
    rakshastra webhook list
    rakshastra webhook remove <name>
    rakshastra webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.rakshastra/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _rakshastra_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: rakshastra gateway run
"""
...
def webhook_command(args):
    """Entry point for 'rakshastra webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: rakshastra webhook {subscribe|list|remove|test}")
        print("Run 'rakshastra webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (rakshastra gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: rakshastra webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from rakshastra webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (rakshastra gateway run)")
