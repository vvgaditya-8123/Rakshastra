"""
Configuration management for Rakshastra Agent.

Config files are stored in ~/.rakshastra/ for easy access:
- ~/.rakshastra/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.rakshastra/.env         - API keys and secrets

This module provides:
- rakshastra config          - Show current configuration
- rakshastra config edit     - Open config in editor
- rakshastra config set      - Set a specific value
- rakshastra config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``rakshastra config set`` (which rewrites
    ...
    ``config.yaml`` in place: rakshastra never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.rakshastra/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``rakshastra logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  rakshastra config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Rakshastra makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Rakshastra itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``rakshastra update``, the TUI build.
...
#   on every plugin install / ``rakshastra update``.
...
# * ``RAKSHASTRA_HOME`` / ``RAKSHASTRA_PROFILE`` / ``RAKSHASTRA_CONFIG`` /
#   ``RAKSHASTRA_ENV`` — Rakshastra runtime location flags. Writing these into
...
#
# IMPORTANT: ``RAKSHASTRA_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (RAKSHASTRA_LANGFUSE_PUBLIC_KEY,
# RAKSHASTRA_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Rakshastra runtime location — never via dashboard env writer.
    # NOT a RAKSHASTRA_* blanket: integration credentials (RAKSHASTRA_GEMINI_*,
    # RAKSHASTRA_LANGFUSE_*, RAKSHASTRA_SPOTIFY_*, ...) ARE allowed.
    "RAKSHASTRA_HOME", "RAKSHASTRA_PROFILE", "RAKSHASTRA_CONFIG", "RAKSHASTRA_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Rakshastra runtime location "
            "(RAKSHASTRA_HOME, RAKSHASTRA_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.rakshastra/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "RAKSHASTRA_TOOL_PROGRESS", "RAKSHASTRA_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `rakshastra plugins enable
    # observability/langfuse` or `rakshastra tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "RAKSHASTRA_LANGFUSE_ENV",
    "RAKSHASTRA_LANGFUSE_RELEASE",
    "RAKSHASTRA_LANGFUSE_SAMPLE_RATE",
    "RAKSHASTRA_LANGFUSE_MAX_CHARS",
    "RAKSHASTRA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from rakshastra_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("RAKSHASTRA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_rakshastra_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Rakshastra is running in package-manager-managed mode.

    Two signals: the RAKSHASTRA_MANAGED env var (set by the systemd service),
    or a .managed marker file in RAKSHASTRA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade rakshastra-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``rakshastra_cli/`` — i.e. the git checkout for source
    installs, ``/opt/rakshastra`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$RAKSHASTRA_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Rakshastra was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$RAKSHASTRA_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. RAKSHASTRA_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.rakshastra``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$RAKSHASTRA_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.rakshastra:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$RAKSHASTRA_HOME``, a
    ...
    ``docker`` and ``rakshastra update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/rakshastra-agent`` image bakes a ``docker``
        stamp into ``/opt/rakshastra`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $RAKSHASTRA_HOME.
    try:
    ...
    #    host install whose shared $RAKSHASTRA_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``rakshastra update``.
    try:
        method = (
            (get_rakshastra_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``rakshastra_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$RAKSHASTRA_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/rakshastra`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Rakshastra lives in a ``uv tool`` layout.

    ``uv tool install rakshastra-agent`` places the install at
    ``.../uv/tools/rakshastra-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``rakshastra-agent`` happens to be uv-tool-installed on the machine while
    the *active* Rakshastra is a regular pip/venv install, causing
    ``rakshastra update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/rakshastra-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade rakshastra-agent"
    if method == "docker":
        return "docker pull nousresearch/rakshastra-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade rakshastra-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade rakshastra-agent"
        return "pip install --upgrade rakshastra-agent"
    return "rakshastra update"
...
# Long-form text for ``rakshastra update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# rakshastra_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Rakshastra, it doesn't update
...
✗ ``rakshastra update`` doesn't apply inside the Docker container.

Rakshastra Agent runs as a published image (nousresearch/rakshastra-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/rakshastra-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate rakshastra-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/rakshastra-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/rakshastra-agent/tags
  • Your config and session history live under ``$RAKSHASTRA_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``rakshastra update`` inside Docker.
...
def format_managed_message(action: str = "modify this Rakshastra installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("RAKSHASTRA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Rakshastra installation is managed by NixOS "
            f"(RAKSHASTRA_MANAGED={env_hint}).\n"
            "Edit services.rakshastra-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Rakshastra installation is managed by Homebrew "
            f"(RAKSHASTRA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade rakshastra-agent"
        )

    return (
        f"Cannot {action}: this Rakshastra installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Rakshastra."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from RAKSHASTRA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, rakshastra_bin
    or None if container mode is not active, we're already inside the
    container, or RAKSHASTRA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("RAKSHASTRA_DEV") == "1":
        return None
    ...
    container_mode_file = get_rakshastra_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "rakshastra-agent")
    exec_user = info.get("exec_user", "rakshastra")
    rakshastra_bin = info.get("rakshastra_bin", "/data/current-package/bin/rakshastra")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "rakshastra_bin": rakshastra_bin,
    }
...
# Re-export from rakshastra_constants — canonical definition lives there.
from rakshastra_constants import get_rakshastra_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_rakshastra_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_rakshastra_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_rakshastra_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the RAKSHASTRA_UID / RAKSHASTRA_GID env vars set by Docker deployments.

    Docker containers running Rakshastra commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level RAKSHASTRA_HOME once, but
    subdirectories created at runtime by ``ensure_rakshastra_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("RAKSHASTRA_UID", "").strip()
    gid_str = os.environ.get("RAKSHASTRA_GID", "").strip()
    try:
...
def _chown_to_rakshastra_uid(path) -> None:
    """Chown ``path`` to ``RAKSHASTRA_UID:RAKSHASTRA_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_rakshastra_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_rakshastra_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the rakshastra group can
    share state with the gateway service.

    The mode can be overridden via the RAKSHASTRA_HOME_MODE environment variable
    (e.g. RAKSHASTRA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse RAKSHASTRA_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``RAKSHASTRA_UID``/``RAKSHASTRA_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("RAKSHASTRA_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_rakshastra_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Rakshastra runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("RAKSHASTRA_CONTAINER") or os.environ.get("RAKSHASTRA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set RAKSHASTRA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into RAKSHASTRA_HOME, upgrading legacy empty templates.
...
def ensure_rakshastra_home():
    """Ensure ~/.rakshastra directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_rakshastra_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_rakshastra_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"RAKSHASTRA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Rakshastra-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Rakshastra
        ...
        # identity slot (SOUL.md). Empty by default. The RAKSHASTRA_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Rakshastra adds a coding operating brief
    ...
    "terminal": {
        ...
        #             RAKSHASTRA_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force RAKSHASTRA_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Rakshastra auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Rakshastra sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Rakshastra
        ...
        #  "/home/user/.rakshastra/cache/documents:/output"]
        ...
        # bundled Rakshastra image, which drops to the `rakshastra` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Rakshastra sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Rakshastra should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``rakshastra chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.rakshastra/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: rakshastra sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .rakshastra.md, or .cursorrules before Rakshastra applies
    ...
    # single read_file page exceeds these limits, Rakshastra truncates the
    ...
    # See tools/kanban_tools.py and rakshastra_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``rakshastra dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``rakshastra kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``rakshastra kanban decompose`` and the kanban
        ...
        # ``rakshastra profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `rakshastra model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `rakshastra` (and `rakshastra chat`) launches by default:
        ...
        # REPL and `--tui` (or RAKSHASTRA_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `rakshastra --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `rakshastra -c` muscle memory.  Default off so existing
        # users aren't surprised.  RAKSHASTRA_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `rakshastra --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/rakshastra`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `rakshastra pets`. Disabled until
        ...
        "pet": {
            ...
            # get_rakshastra_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``RAKSHASTRA_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``RAKSHASTRA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``RAKSHASTRA_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``RAKSHASTRA_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``RAKSHASTRA_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/rakshastra``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Rakshastra appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.rakshastra/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.rakshastra/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.rakshastra/cache/delegation/
    ...
    # response. If not, Rakshastra feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Rakshastra auto-pauses the goal and
    ...
    "moa": {
        ...
        # <rakshastra_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.rakshastra/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${RAKSHASTRA_SKILL_DIR} and ${RAKSHASTRA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `rakshastra curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `rakshastra curator run --consolidate` overrides
        ...
        # normally restored on every `rakshastra update`, so pruning them only
        ...
        # skipped), snapshot ~/.rakshastra/skills/ into
        # ~/.rakshastra/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `rakshastra curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for rakshastra-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Rakshastra Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (RAKSHASTRA_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Rakshastra' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.rakshastra/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or RAKSHASTRA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `rakshastra doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``rakshastra_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Rakshastra to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $RAKSHASTRA_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via RAKSHASTRA_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `rakshastra -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `rakshastra` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `rakshastra kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     rakshastra-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Rakshastra tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.rakshastra/logs/.
    ...
    # update model picker lists without shipping a rakshastra-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://rakshastra-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `rakshastra model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal RAKSHASTRA_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # the RAKSHASTRA_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.rakshastra.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.rakshastra/.env,
        ...
        # behavior: files must live under the Rakshastra cache, under
        ...
        # user. Bridged to RAKSHASTRA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Rakshastra
        # cache (~/.rakshastra/cache/{documents,images,audio,video,screenshots})
        ...
        # to RAKSHASTRA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # RAKSHASTRA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to RAKSHASTRA_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.rakshastra/state.db.
    ...
    "sessions": {
        ...
        # default of ``rakshastra sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.rakshastra/sessions/session_{sid}.json`` on every turn
    ...
    # ``rakshastra update`` behaviour.
    "updates": {
        # Run a full ``rakshastra backup``-style zip of RAKSHASTRA_HOME before every
        # ``rakshastra update``.  Backups land in ``<RAKSHASTRA_HOME>/backups/`` and
        # can be restored with ``rakshastra import <path>``.  Off by default:
        # zipping a large RAKSHASTRA_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``rakshastra update
        ...
        "backup_keep": 5,
        # What `rakshastra update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<RAKSHASTRA_HOME>/lsp/bin/`` on first use.
    ...
    # `rakshastra tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.rakshastra/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.rakshastra/.env (or your shell) and never in config.yaml.
            ...
            # ~/.rakshastra/bin/ on first use.  When False you must install
            ...
            # `rakshastra secrets bitwarden setup`.
            "server_url": "",
        },
    },
    ...
    "computer_use": {
        ...
        # by default upstream. Rakshastra disables it for our users unless they
        # explicitly opt in here. When false (default), Rakshastra sets
    ...
    # Rakshastra Desktop (Electron app) launch options. These only affect
    # `rakshastra desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the RAKSHASTRA_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'rakshastra model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Rakshastra auth store)",
    ...
    "RAKSHASTRA_LANGFUSE_PUBLIC_KEY": {
    ...
    "RAKSHASTRA_LANGFUSE_SECRET_KEY": {
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Rakshastra, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @rakshastra:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. RAKSHASTRA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #rakshastra)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: rakshastra-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'rakshastra-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Rakshastra API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Rakshastra API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Rakshastra API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # RAKSHASTRA_TOOL_PROGRESS and RAKSHASTRA_TOOL_PROGRESS_MODE are deprecated —
    ...
    "RAKSHASTRA_PREFILL_MESSAGES_FILE": {
    ...
    "RAKSHASTRA_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.rakshastra.config`` entries, then checks
    ...
    try:
        ...
        # should never break `rakshastra update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Rakshastra versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``rakshastra_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'rakshastra setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Rakshastra won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'rakshastra doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("RAKSHASTRA_HOME", "~/.rakshastra")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "rakshastra update / rakshastra -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("RAKSHASTRA_TOOL_PROGRESS")
            old_mode = get_env_value("RAKSHASTRA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from RAKSHASTRA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from RAKSHASTRA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("RAKSHASTRA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from RAKSHASTRA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$RAKSHASTRA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_rakshastra_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `rakshastra plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `rakshastra curator status` has no
    ...
    #   3. Creates `~/.rakshastra/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_rakshastra_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_rakshastra_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `rakshastra config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `rakshastra config set`"
                    )
    ...
    # behavior set it to true explicitly via `rakshastra config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from rakshastra_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: rakshastra config set <key> <value>")
    ...
    # `rakshastra update`, but only the version bump is persisted.
    ...
    # metadata.rakshastra.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``rakshastra config set``
    ...
    backends) — while display paths (``rakshastra status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _rakshastra_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.rakshastra/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.rakshastra/config.yaml.
    ...
    (which change ``RAKSHASTRA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/rakshastra/config.yaml invalidates the
...
#   openai-codex (OAuth — rakshastra auth) — OpenAI Codex
#   nous         (OAuth — rakshastra auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.rakshastra/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Rakshastra writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.rakshastra/.env.
    ...
    interactive menu render (`rakshastra tools`, `rakshastra setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `rakshastra tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Rakshastra env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.rakshastra/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.rakshastra/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.rakshastra/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.rakshastra/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Rakshastra — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Rakshastra vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.rakshastra/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.rakshastra/.env`` over ``os.environ``.

    Used for Rakshastra-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Rakshastra Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Warn on stale RAKSHASTRA_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("RAKSHASTRA_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale RAKSHASTRA_MAX_ITERATIONS={_env_ghost} "
                f"(run 'rakshastra doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  rakshastra config edit     # Edit config file", Colors.DIM))
    print(color("  rakshastra config set <key> <value>", Colors.DIM))
    print(color("  rakshastra setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `rakshastra config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `rakshastra config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: rakshastra config set <key> <value>")
            print()
            print("Examples:")
            print("  rakshastra config set model anthropic/claude-sonnet-4")
            print("  rakshastra config set terminal.backend docker")
            print("  rakshastra config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'rakshastra config migrate' to add them")
        ...
        print("  rakshastra config           Show current configuration")
        print("  rakshastra config edit      Open config in editor")
        print("  rakshastra config set <key> <value>   Set a config value")
        print("  rakshastra config check     Check for missing/outdated config")
        print("  rakshastra config migrate   Update config with new options")
        print("  rakshastra config path      Show config file path")
        print("  rakshastra config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``rakshastra config`` UI so users
