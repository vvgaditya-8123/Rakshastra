"""
Rakshastra Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``rakshastra desktop`` / ``rakshastra gui``)
     Built inside the agent checkout under ``$RAKSHASTRA_HOME/rakshastra-agent/``:
       ...
       - ``$RAKSHASTRA_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Rakshastra.app`` or ``~/Applications/Rakshastra.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Rakshastra`` (NSIS per-user)
...
the app name ("Rakshastra"), separate from ``$RAKSHASTRA_HOME``:
  - macOS:   ``~/Library/Application Support/Rakshastra``
  - Windows: ``%APPDATA%\\Rakshastra``
  - Linux:   ``$XDG_CONFIG_HOME/Rakshastra`` (default ``~/.config/Rakshastra``)
...
``rakshastra uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(rakshastra_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return rakshastra_home / "rakshastra-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Rakshastra"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Rakshastra"
...
def source_built_gui_artifacts(rakshastra_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``rakshastra desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``rakshastra-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(rakshastra_home)
    ...
    return [
        ...
        rakshastra_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Rakshastra" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Rakshastra.app"),
            home / "Applications" / "Rakshastra.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Rakshastra).
            local_base / "Programs" / "Rakshastra",
            # Older / alternate layout some builds used.
            local_base / "rakshastra-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Rakshastra")
    else:
        ...
        paths += [
            data_base / "applications" / "rakshastra.desktop",
            data_base / "applications" / "Rakshastra.desktop",
        ]
    return paths


def agent_is_installed(rakshastra_home: Path) -> bool:
    """Return True when a usable Python agent install exists under RAKSHASTRA_HOME.
    ...
    if (agent_root / "rakshastra_cli").is_dir():
        return True
...
def gui_is_installed(rakshastra_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(rakshastra_home):
        if p.exists():
            return True
...
def gui_install_summary(rakshastra_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = rakshastra_home if rakshastra_home is not None else get_rakshastra_home()
...
def uninstall_gui(rakshastra_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``rakshastra-agent/rakshastra_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$RAKSHASTRA_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove rakshastra' or "
            "'sudo dnf remove rakshastra'). AppImage builds are a single file you "
