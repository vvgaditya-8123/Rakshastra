"""``rakshastra logs`` — view and filter Rakshastra log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.rakshastra/logs/``.

Usage examples::

    rakshastra logs                    # last 50 lines of agent.log
    rakshastra logs -f                 # follow agent.log in real time
    rakshastra logs errors             # last 50 lines of errors.log
    rakshastra logs gateway -n 100    # last 100 lines of gateway.log
    rakshastra logs gui -f            # follow gui.log (dashboard/pty/ws)
    rakshastra logs desktop -f        # follow desktop.log (Electron app boot/backend)
    rakshastra logs --level WARNING    # only WARNING+ lines
    rakshastra logs --session abc123   # filter by session ID substring
    rakshastra logs --component tools  # only tool-related lines
    rakshastra logs --since 1h         # lines from the last hour
    rakshastra logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_rakshastra_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Rakshastra runs — try 'rakshastra chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_rakshastra_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_rakshastra_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_rakshastra_home()}/logs/")
        return

    print(f"Log files in {display_rakshastra_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'rakshastra chat' to generate logs)")
