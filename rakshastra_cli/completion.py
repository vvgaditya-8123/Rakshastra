"""Shell completion script generation for rakshastra CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_rakshastra_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Rakshastra Agent bash completion
# Add to ~/.bashrc:
#   eval "$(rakshastra completion bash)"

_rakshastra_profiles() {{
    local profiles_dir="$HOME/.rakshastra/profiles"
...
_rakshastra_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_rakshastra_profiles)" -- "$cur"))
        return
    fi
...
complete -F _rakshastra_completion rakshastra
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef rakshastra
# Rakshastra Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(rakshastra completion zsh)"

_rakshastra_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.rakshastra/profiles" ]]; then
        profiles+=($HOME/.rakshastra/profiles/*(N/:t))
    fi
...
_rakshastra() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_rakshastra_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'rakshastra command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
compdef _rakshastra rakshastra
"""
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Rakshastra Agent fish completion",
        "# Add to your config:",
        "#   rakshastra completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __rakshastra_profiles",
        "    echo default",
        "    if test -d $HOME/.rakshastra/profiles",
        "        for d in $HOME/.rakshastra/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c rakshastra -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c rakshastra -f -s p -l profile"
        " -d 'Profile name' -xa '(__rakshastra_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__rakshastra_profiles)' -d 'Profile name'"
                )
