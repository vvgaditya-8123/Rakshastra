...
on Japanese locales, etc.).  Rakshastra's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``rakshastra_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Rakshastra is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``RAKSHASTRA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("RAKSHASTRA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Rakshastra-managed Git install directories so
    ...
    # already-running shells don't see the change — which means rakshastra
    ...
    # install without Rakshastra-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Rakshastra's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Rakshastra's
    ``rakshastra config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Rakshastra (User env var in Windows Settings, or
    export in a PowerShell profile) and Rakshastra picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Rakshastra-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\rakshastra\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``rakshastra`` from right after install) retain their old PATH.

    Any subprocess Rakshastra spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Rakshastra-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "rakshastra", "git", "cmd"),
        os.path.join(local_appdata, "rakshastra", "git", "bin"),
        os.path.join(local_appdata, "rakshastra", "git", "usr", "bin"),
        # Rakshastra venv Scripts directory — host of the rakshastra.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes rakshastra, but harmless to include.
        os.path.join(local_appdata, "rakshastra", "rakshastra-agent", "venv", "Scripts"),
