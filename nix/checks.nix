...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      rakshastra-agent = self'.packages.default;
      rakshastraVenv = rakshastra-agent.rakshastraVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "rakshastra-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${rakshastraVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "rakshastra-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "rakshastra-build-package" { } ''
          echo "PASS: package built at ${rakshastra-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "rakshastra-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "rakshastra-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${rakshastra-agent}/bin/rakshastra || (echo "FAIL: rakshastra binary missing"; exit 1)
          test -x ${rakshastra-agent}/bin/rakshastra-agent || (echo "FAIL: rakshastra-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${rakshastra-agent}/bin/rakshastra version 2>&1 | grep -qi "rakshastra" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "rakshastra-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in rakshastra rakshastra-agent rakshastra-acp; do
            test -x ${rakshastra-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "rakshastra-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking rakshastra --help ==="
          ${rakshastra-agent}/bin/rakshastra --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${rakshastra-agent}/bin/rakshastra --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "rakshastra-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${rakshastra-agent}/share/rakshastra-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${rakshastra-agent}/share/rakshastra-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "RAKSHASTRA_BUNDLED_SKILLS" ${rakshastra-agent}/bin/rakshastra || \
            (echo "FAIL: RAKSHASTRA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: RAKSHASTRA_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "rakshastra-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${rakshastra-agent}/share/rakshastra-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${rakshastra-agent}/share/rakshastra-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "RAKSHASTRA_BUNDLED_PLUGINS" ${rakshastra-agent}/bin/rakshastra || \
            (echo "FAIL: RAKSHASTRA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: RAKSHASTRA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "rakshastra-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${rakshastra-agent}/share/rakshastra-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${rakshastra-agent}/share/rakshastra-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${rakshastra-agent}/share/rakshastra-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "RAKSHASTRA_BUNDLED_LOCALES" ${rakshastra-agent}/bin/rakshastra || \
            (echo "FAIL: RAKSHASTRA_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: RAKSHASTRA_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (RAKSHASTRA_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && RAKSHASTRA_BUNDLED_LOCALES=${rakshastra-agent}/share/rakshastra-agent/locales \
            ${rakshastraVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with RAKSHASTRA_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${rakshastraVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${rakshastraVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without RAKSHASTRA_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "rakshastra-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${rakshastra-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${rakshastra-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "RAKSHASTRA_TUI_DIR" ${rakshastra-agent}/bin/rakshastra || \
            (echo "FAIL: RAKSHASTRA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: RAKSHASTRA_TUI_DIR set in wrapper"
        ...
        # Verify RAKSHASTRA_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        rakshastra-node = pkgs.runCommand "rakshastra-node-version" { } ''
          set -e
          echo "=== Checking RAKSHASTRA_NODE in wrapper ==="
          grep -q "RAKSHASTRA_NODE" ${rakshastra-agent}/bin/rakshastra || \
            (echo "FAIL: RAKSHASTRA_NODE not set in wrapper"; exit 1)
          echo "PASS: RAKSHASTRA_NODE present in wrapper"

          RAKSHASTRA_NODE=$(sed -n "s/^export RAKSHASTRA_NODE='\(.*\)'/\1/p" ${rakshastra-agent}/bin/rakshastra)
          test -x "$RAKSHASTRA_NODE" || (echo "FAIL: RAKSHASTRA_NODE=$RAKSHASTRA_NODE not executable"; exit 1)
          echo "PASS: RAKSHASTRA_NODE executable at $RAKSHASTRA_NODE"

          NODE_MAJOR=$("$RAKSHASTRA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All RAKSHASTRA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify RAKSHASTRA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "rakshastra-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(RAKSHASTRA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking RAKSHASTRA_MANAGED guards ==="
          check_blocked "config set" ${rakshastra-agent}/bin/rakshastra config set model foo
          check_blocked "config edit" ${rakshastra-agent}/bin/rakshastra config edit
        ...
        extra-python-packages = let
          ...
          rakshastraWithExtra = rakshastra-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "rakshastra-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${rakshastraWithExtra}/bin/rakshastra || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${rakshastraWithExtra}/bin/rakshastra || \
          ...
          if grep -q "PYTHONPATH" ${rakshastra-agent}/bin/rakshastra; then
        ...
        extra-dependency-groups = let
          rakshastraWithGroups = rakshastra-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "rakshastra-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${rakshastraWithGroups}"
          echo "venv: ${rakshastraWithGroups.rakshastraVenv}"
        ...
        messaging-variant = pkgs.runCommand "rakshastra-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.rakshastraVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "rakshastra-config-roundtrip" {
          ...
          merge_and_load() {
            local rakshastra_home="$1"
            export RAKSHASTRA_HOME="$rakshastra_home"
            ${configMergeScript} ${nixSettings} "$rakshastra_home/config.yaml"
            ${rakshastraVenv}/bin/python3 -c '
import json, sys
