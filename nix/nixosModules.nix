# nix/nixosModules.nix — NixOS module for rakshastra-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: rakshastra runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $RAKSHASTRA_HOME/.env
# and read by rakshastra at startup — no container recreation needed for env changes.
#
# Tool resolution: the rakshastra wrapper uses --suffix PATH for nix store tools,
...
#
# Usage:
#   services.rakshastra-agent = {
#     enable = true;
#     settings.model = "anthropic/claude-sonnet-4";
#     environmentFiles = [ config.sops.secrets."rakshastra/env".path ];
#   };
#
{ inputs, ... }: {
  ...
  let
    cfg = config.services.rakshastra-agent;
    effectivePackage =
    ...
    rakshastra-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-rakshastra-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "rakshastra-config-attrs";
      description = "Rakshastra YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "rakshastra-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # RAKSHASTRA_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "rakshastra-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'RAKSHASTRA_DOC_EOF'\n${value}\nRAKSHASTRA_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "rakshastra-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/rakshastra";
    ...
    # rakshastra user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "rakshastra-container-entrypoint" ''
      set -eu

      RAKSHASTRA_UID="''${RAKSHASTRA_UID:?RAKSHASTRA_UID must be set}"
      RAKSHASTRA_GID="''${RAKSHASTRA_GID:?RAKSHASTRA_GID must be set}"

      # ── Group: ensure a group with GID=$RAKSHASTRA_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$RAKSHASTRA_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="rakshastra"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$RAKSHASTRA_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$RAKSHASTRA_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$RAKSHASTRA_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$RAKSHASTRA_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="rakshastra"
        TARGET_HOME="/home/rakshastra"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$RAKSHASTRA_UID" -g "$RAKSHASTRA_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$RAKSHASTRA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$RAKSHASTRA_UID:$RAKSHASTRA_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure RAKSHASTRA_HOME is owned by the target user.
      ...
      if [ -n "''${RAKSHASTRA_HOME:-}" ] && [ -d "$RAKSHASTRA_HOME" ]; then
        find "$RAKSHASTRA_HOME" \! -user "$RAKSHASTRA_UID" -exec chown "$RAKSHASTRA_UID:$RAKSHASTRA_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/rakshastra-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/rakshastra-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/rakshastra ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/rakshastra
        chmod 0440 /etc/sudoers.d/rakshastra
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$RAKSHASTRA_UID" --regid="$RAKSHASTRA_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $RAKSHASTRA_HOME/.env.
    ...
    # Default: /var/lib/rakshastra/workspace → /data/workspace.
  ...
  in {
    options.services.rakshastra-agent = with lib; {
      enable = mkEnableOption "Rakshastra Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = rakshastra-agent;
        description = "The rakshastra-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/rakshastra";
        description = "State directory. Contains .rakshastra/ subdir (RAKSHASTRA_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Rakshastra config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $RAKSHASTRA_HOME/.env at activation time.
          Rakshastra reads this file on every startup via load_rakshastra_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $RAKSHASTRA_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $RAKSHASTRA_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `rakshastra gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the rakshastra user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the rakshastra plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Rakshastra discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          rakshastra_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as rakshastra (python312).
        '';
        example = literalExpression ''
          [
            (pkgs.python312Packages.buildPythonPackage {
              pname = "rtk-rakshastra";
              version = "1.0.0";
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in rakshastra-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the rakshastra CLI to environment.systemPackages and export
          RAKSHASTRA_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.rakshastra symlink to the service
            stateDir. These users are automatically added to the rakshastra group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.rakshastra-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the rakshastra CLI to system PATH and export RAKSHASTRA_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.rakshastra/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.RAKSHASTRA_HOME = "${cfg.stateDir}/.rakshastra";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.rakshastra-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the rakshastra user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.rakshastra-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed rakshastra
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure rakshastra is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.rakshastra        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.rakshastra/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.rakshastra/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.rakshastra/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.rakshastra/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.rakshastra/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."rakshastra-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.rakshastra
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.rakshastra ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.rakshastra ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.rakshastra -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.rakshastra/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.rakshastra/$_subdir"
            chmod 2770 "${cfg.stateDir}/.rakshastra/$_subdir"
            find "${cfg.stateDir}/.rakshastra/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # rakshastra-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.rakshastra/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.rakshastra/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.rakshastra/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.rakshastra/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.rakshastra/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.rakshastra/.managed
          chmod 0644 ${cfg.stateDir}/.rakshastra/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.rakshastra/.container-mode <<'RAKSHASTRA_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    rakshastra_bin=${containerDataDir}/current-package/bin/rakshastra
    RAKSHASTRA_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.rakshastra/.container-mode
            chmod 0644 ${cfg.stateDir}/.rakshastra/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.rakshastra/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.rakshastra";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.rakshastra" ]; then
                  rm -f "${symlinkPath}"
                  echo "rakshastra-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.rakshastra -> stateDir/.rakshastra for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.rakshastra";
                target = "${cfg.stateDir}/.rakshastra";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "rakshastra-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.rakshastra/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.rakshastra/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.rakshastra/auth.json
              fi
            ''}
          ''}
          ...
          # Rakshastra reads $RAKSHASTRA_HOME/.env at startup via load_rakshastra_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.rakshastra/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'RAKSHASTRA_NIX_ENV_EOF'
    ${envFileContent}
    RAKSHASTRA_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
        ...
        find ${cfg.stateDir}/.rakshastra/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.rakshastra/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.rakshastra/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.rakshastra-agent = {
          description = "Rakshastra Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            RAKSHASTRA_HOME = "${cfg.stateDir}/.rakshastra";
            RAKSHASTRA_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $RAKSHASTRA_HOME/.env by the activation script. load_rakshastra_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/rakshastra"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the rakshastra group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.rakshastra-agent = {
          description = "Rakshastra Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              RAKSHASTRA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              RAKSHASTRA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env RAKSHASTRA_UID="$RAKSHASTRA_UID" \
                --env RAKSHASTRA_GID="$RAKSHASTRA_GID" \
                --env RAKSHASTRA_HOME=${containerDataDir}/.rakshastra \
                --env RAKSHASTRA_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/rakshastra gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
