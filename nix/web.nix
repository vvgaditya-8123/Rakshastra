# nix/web.nix — Rakshastra Web Dashboard (Vite/React) frontend build
{ pkgs, rakshastraNpmLib, ... }:
let
  npm = rakshastraNpmLib.mkNpmPassthru { folder = "web"; attr = "web"; pname = "rakshastra-web"; };

  packageJson = builtins.fromJSON (builtins.readFile (npm.src + "/web/package.json"));
  version = packageJson.version;
in
pkgs.buildNpmPackage (npm // {
  pname = "rakshastra-web";
  inherit version;
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../rakshastra_cli/web_dist for the
