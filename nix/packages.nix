# nix/packages.nix — Rakshastra Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      minimal = pkgs.callPackage ./rakshastra-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.rakshastraTui;
        web = full.rakshastraWeb;
        desktop = full.rakshastraDesktop;
      };
    };
}
