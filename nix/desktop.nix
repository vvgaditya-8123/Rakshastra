# nix/desktop.nix — Rakshastra Desktop (Electron) app build + wrapper
#
# `rakshastraAgent` is the fully-built `.#default` package — it ships the
# `rakshastra` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `RAKSHASTRA_DESKTOP_RAKSHASTRA` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Rakshastra CLI").
...
let
  npm = rakshastraNpmLib.mkNpmPassthru {
    folder = "apps/desktop";
    attr = "desktop";
    pname = "rakshastra-desktop";
  };
  ...
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "rakshastra-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/rakshastra-desktop $out/bin
    cp -r ${renderer}/* $out/share/rakshastra-desktop/
    ...
    substituteInPlace $out/share/rakshastra-desktop/electron/main.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/rakshastra-desktop'"
    ...
    substituteInPlace $out/share/rakshastra-desktop/electron/git-review-ops.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/rakshastra-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # RAKSHASTRA_DESKTOP_RAKSHASTRA to the absolute path of the nix-built `rakshastra`
    # binary so the desktop's resolver step 4 ("existing Rakshastra CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/rakshastra-desktop \
      --add-flags "$out/share/rakshastra-desktop" \
      --set RAKSHASTRA_DESKTOP_RAKSHASTRA "${lib.getExe rakshastraAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Rakshastra Agent";
    homepage = "https://github.com/NousResearch/rakshastra-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "rakshastra-desktop";
  };
}
