...
- Skill helper paths:
  - `${RAKSHASTRA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${RAKSHASTRA_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${RAKSHASTRA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client rakshastra
```
...
```bash
PUBLISH="${RAKSHASTRA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client rakshastra
```
...
```bash
DRIVE="${RAKSHASTRA_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `rakshastra`)    |
