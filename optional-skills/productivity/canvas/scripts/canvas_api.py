#!/usr/bin/env python3
"""Canvas LMS API CLI for Rakshastra Agent.
...
def _check_config():
    ...
    if missing:
        rakshastra_env = os.path.join(
            os.environ.get("RAKSHASTRA_HOME", os.path.expanduser("~/.rakshastra")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {rakshastra_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Rakshastra Agent"
    )
