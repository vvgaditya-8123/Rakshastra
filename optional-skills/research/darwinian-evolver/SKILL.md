...
version: 0.1.0
author: Bihruze (Asahi0x), Rakshastra Agent
license: MIT
...
import upstream classes into Rakshastra itself.
...
```bash
mkdir -p ~/.rakshastra/cache/darwinian-evolver && cd ~/.rakshastra/cache/darwinian-evolver
...
```bash
cd ~/.rakshastra/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.rakshastra/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.rakshastra/skills/research/darwinian-evolver
DE_DIR=~/.rakshastra/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Rakshastra core.
   Custom driver scripts under `~/.rakshastra/skills/...` are user-side and fine.
