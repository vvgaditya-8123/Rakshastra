...
- smoke-test a server with the FastMCP CLI before wiring it into Rakshastra or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Rakshastra. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.rakshastra/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.rakshastra/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Rakshastra integration, either:

- configure the server in `~/.rakshastra/config.yaml` using the `native-mcp` skill, or
...
### Rakshastra cannot see the deployed server

The server-building part may be correct while the Rakshastra config is not. Load the `native-mcp` skill and configure the server in `~/.rakshastra/config.yaml`, then restart Rakshastra.
