...
## 5. GODMODE FAST — Rakshastra 4 405B (Zero Refusal)

**Model:** `nousresearch/rakshastra-4-405b`
**Strategy:** Rakshastra models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Rakshastra Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.rakshastra/config.yaml`:
