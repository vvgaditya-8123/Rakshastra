...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("RAKSHASTRA_HOME", os.path.expanduser("~/.rakshastra")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("RAKSHASTRA_HOME", Path.home() / ".rakshastra")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Rakshastra config paths
# ═══════════════════════════════════════════════════════════════════

RAKSHASTRA_HOME = Path(os.getenv("RAKSHASTRA_HOME", Path.home() / ".rakshastra"))
CONFIG_PATH = RAKSHASTRA_HOME / "config.yaml"
PREFILL_PATH = RAKSHASTRA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Rakshastra models — already uncensored, just needs clean prompt
    "rakshastra": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "rakshastra" in model_lower or "nous" in model_lower:
        return "rakshastra"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Rakshastra config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.rakshastra/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Rakshastra for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Rakshastra for changes to take effect.")
