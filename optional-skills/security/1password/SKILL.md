...
version: 1.0.0
author: arceus77-7, enhanced by Rakshastra Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during Rakshastra terminal calls (desktop app flow only)
...
### Service Account (recommended for Rakshastra)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${RAKSHASTRA_HOME:-~/.rakshastra}/.env` (the skill will prompt for this on first load).
...
## Rakshastra Execution Pattern (desktop app flow)

Rakshastra terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/rakshastra-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/rakshastra-op.sock"
