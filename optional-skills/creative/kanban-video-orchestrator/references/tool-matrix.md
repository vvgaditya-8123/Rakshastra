...
Maps each role archetype to the Rakshastra skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public rakshastra-agent
...
## Rakshastra skills relevant to video production

### Visual / rendering skills (`rakshastra-agent/skills/creative/`)
...
### Design / pre-production skills (`rakshastra-agent/skills/creative/`)
...
### Audio / media skills (`rakshastra-agent/skills/creative/` + `skills/media/`)
...
These are **not** Rakshastra skills but external CLIs / APIs that profiles invoke.
...
## Built-in Rakshastra tools for media review

These are native Rakshastra tools — not invoked via terminal but through their own
...
key is present in `${RAKSHASTRA_HOME:-~/.rakshastra}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Rakshastra profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${RAKSHASTRA_HOME:-~/.rakshastra}/.env` → environment variable.
...
When a new Rakshastra-public video skill ships:
