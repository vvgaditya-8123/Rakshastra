...
the actual `setup.sh` that creates the project workspace, configures Rakshastra
...
2. **Create profiles** — `rakshastra profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.rakshastra/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `rakshastra kanban create` assigned to the director
...
```bash
rakshastra profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.rakshastra/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.rakshastra/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Rakshastra
...
Each profile gets a `SOUL.md` at `~/.rakshastra/profiles/<name>/SOUL.md` that
...
```bash
rakshastra kanban create "Direct production of <video title>" \
    --assignee director \
...
the Rakshastra `.env` (`${RAKSHASTRA_HOME:-$HOME/.rakshastra}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _rakshastra_env="${RAKSHASTRA_HOME:-$HOME/.rakshastra}/.env"
    if grep -q "^${var}=" "$_rakshastra_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_rakshastra_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_rakshastra_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY rakshastra ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY rakshastra OPENROUTER_API_KEY || exit 1
# ...
```
