...
rakshastra kanban watch --tenant <project-slug>

# Snapshot of the board
rakshastra kanban list --tenant <project-slug>
rakshastra kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
rakshastra kanban stats --tenant <project-slug>

# Visual dashboard (browser)
rakshastra dashboard

# Inspect a specific task (includes comments + events)
rakshastra kanban show <task-id>

# Follow a single task's event stream
rakshastra kanban tail <task-id>
```

Verify available subcommands with `rakshastra kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `rakshastra kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `rakshastra kanban block <id>` then `rakshastra kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `rakshastra kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `rakshastra kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `rakshastra profile list` (profile exists?), `rakshastra status` (gateway/dispatcher up?), and verify tenant. |
...
rakshastra kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
rakshastra kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(rakshastra kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
rakshastra kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
rakshastra kanban block <task-id>
rakshastra kanban archive <task-id>

# Diagnose what happened
rakshastra kanban show <task-id>      # task body, comments, recent events
rakshastra kanban tail <task-id>      # follow the live event stream
rakshastra kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `rakshastra kanban create` for the director
...
while true; do
    clear
    rakshastra kanban list --tenant <slug>
    echo "---"
    rakshastra kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `rakshastra kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `rakshastra profile list` and start
