...
Usage (via cron with --no-agent):
    ...
    rakshastra cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$RAKSHASTRA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Rakshastra-Watcher/1.0"})
