...
Usage (via cron with --no-agent):
    ...
    rakshastra cron create rakshastra-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$RAKSHASTRA_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name rakshastra-issues --repo NousResearch/rakshastra-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Rakshastra .env file
(``${RAKSHASTRA_HOME:-~/.rakshastra}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Rakshastra-Watcher/1.0",
    }
