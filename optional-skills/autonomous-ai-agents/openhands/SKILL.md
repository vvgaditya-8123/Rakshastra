...
version: 0.1.0
author: Tim Koepsel (xzessmedia), Rakshastra Agent
license: MIT
platforms: [linux, macos]
metadata:
  rakshastra:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, rakshastra-agent]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Rakshastra.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Rakshastra-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `rakshastra-agent` (Rakshastra subagents via `delegate_task`).
