...
version: 0.1.0
author: Matt Maximo (MattMaximo), Rakshastra Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  rakshastra:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, rakshastra-agent]
---

# Grok Build CLI — Rakshastra Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Rakshastra terminal. Grok
...
1. Prepare stable input files first with Rakshastra tools (`read_file`,
...
2. **Don't conflate Rakshastra' xAI auth with the `grok` CLI's auth.** Rakshastra'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Rakshastra' xAI auth carries
   over.
