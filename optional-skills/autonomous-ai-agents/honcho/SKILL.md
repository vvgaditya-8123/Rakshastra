---
name: honcho
description: Configure and use Honcho memory with Rakshastra -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
...
# Honcho Memory for Rakshastra

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Rakshastra profile its own peer identity while sharing a unified view of the user.
...
```bash
rakshastra memory setup honcho
...
See: https://docs.honcho.dev/v3/guides/integrations/rakshastra#running-honcho-locally-with-rakshastra

### Verify

```bash
rakshastra honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Rakshastra profile's AI peer
...
Honcho models conversations as interactions between **peers**. Rakshastra creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Rakshastra instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Rakshastra run |
| `global` | Single session across all directories |

Manual override: `rakshastra honcho map my-project-name`
...
Each Rakshastra profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
rakshastra profile create coder --clone
# creates host block rakshastra.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `rakshastra.coder` host block in `honcho.json`
...
```bash
rakshastra honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Rakshastra when Honcho memory is active.
...
Config file: `$RAKSHASTRA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `rakshastra honcho setup`. Ensure `memory.provider: honcho` is in `~/.rakshastra/config.yaml`.

### Memory not persisting across sessions
Check `rakshastra honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `rakshastra profile create <name> --clone`. For existing profiles: `rakshastra honcho sync`.
...
| Command | Description |
|---------|-------------|
| `rakshastra honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `rakshastra honcho status` | Show resolved config, connection test, peer info for active profile |
| `rakshastra honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `rakshastra honcho disable` | Disable Honcho for the active profile |
| `rakshastra honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `rakshastra honcho peers` | Show peer identities across all profiles |
| `rakshastra honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `rakshastra honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `rakshastra honcho sessions` | List known directory-to-session-name mappings |
| `rakshastra honcho map <name>` | Map current working directory to a Honcho session name |
| `rakshastra honcho identity` | Seed AI peer identity or show both peer representations |
| `rakshastra honcho sync` | Create host blocks for all Rakshastra profiles that don't have one yet |
| `rakshastra honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Rakshastra + Honcho |
| `rakshastra memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `rakshastra memory status` | Show active memory provider and config |
| `rakshastra memory off` | Disable external memory provider |
