"""
Rakshastra MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Rakshastra-specific extra)

Usage:
    rakshastra mcp serve
    rakshastra mcp serve --verbose
...
logger = logging.getLogger("rakshastra.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using RAKSHASTRA_HOME."""
    try:
        from rakshastra_constants import get_rakshastra_home
        return get_rakshastra_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("RAKSHASTRA_HOME", Path.home() / ".rakshastra")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from rakshastra_constants import get_rakshastra_home
        directory_file = get_rakshastra_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("RAKSHASTRA_HOME", Path.home() / ".rakshastra")
...
class EventBridge:
    ...
    This is the Rakshastra equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from rakshastra_constants import get_rakshastra_home
            db_file = get_rakshastra_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("RAKSHASTRA_HOME", Path.home() / ".rakshastra")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Rakshastra MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "rakshastra",
        instructions=(
            "Rakshastra Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Rakshastra MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
