...
4. Store the token in `${RAKSHASTRA_HOME:-~/.rakshastra}/.env` (or via `rakshastra setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Rakshastra. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Important Notes for Rakshastra
...
- **`AIRTABLE_API_KEY` flows from `${RAKSHASTRA_HOME:-~/.rakshastra}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
