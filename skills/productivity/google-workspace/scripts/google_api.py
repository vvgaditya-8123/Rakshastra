#!/usr/bin/env python3
"""Google Workspace API CLI for Rakshastra Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Rakshastra-facing JSON contract and falls back to the Python client
...
from _rakshastra_home import get_rakshastra_home
...
def _gws_binary() -> str | None:
    override = os.getenv("RAKSHASTRA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Rakshastra Agent")
