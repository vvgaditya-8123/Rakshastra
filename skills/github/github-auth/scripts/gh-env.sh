#!/usr/bin/env bash
# GitHub environment detection helper for Rakshastra Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _rakshastra_env="${RAKSHASTRA_HOME:-$HOME/.rakshastra}/.env"; [ -f "$_rakshastra_env" ] && grep -q "^GITHUB_TOKEN=" "$_rakshastra_env" 2>/dev/null; then
