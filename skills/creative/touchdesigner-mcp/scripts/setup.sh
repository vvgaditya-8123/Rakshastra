...
RAKSHASTRA_HOME_DIR="${RAKSHASTRA_HOME:-$HOME/.rakshastra}"
RAKSHASTRA_CFG="${RAKSHASTRA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Rakshastra config has twozero_td MCP entry ──
if [[ ! -f "$RAKSHASTRA_CFG" ]]; then
    echo -e " ${FAIL} Rakshastra config not found at ${RAKSHASTRA_CFG}"
    manual_steps+=("Create ${RAKSHASTRA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$RAKSHASTRA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Rakshastra config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Rakshastra config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$RAKSHASTRA_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${RAKSHASTRA_CFG} manually"); }
    manual_steps+=("Restart Rakshastra session to pick up config change")
fi
