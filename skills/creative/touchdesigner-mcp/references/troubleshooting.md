...
## 5. Rakshastra Configuration

### Config location

`$RAKSHASTRA_HOME/config.yaml` (defaults to `~/.rakshastra/config.yaml` when `RAKSHASTRA_HOME` is unset)
...
Restart the Rakshastra session for changes to take effect. The MCP connection is
