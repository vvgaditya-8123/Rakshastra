...
metadata:
  rakshastra:
    ...
    related_skills: [claude-code, rakshastra-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Rakshastra terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Rakshastra itself, `model.provider: openai-codex` uses Rakshastra-managed Codex
OAuth from `~/.rakshastra/auth.json` after `rakshastra auth add openai-codex`. For the
...
## Rakshastra Gateway Caveat

When invoking the Codex CLI from a Rakshastra gateway/service context (for example,
