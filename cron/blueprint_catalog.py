...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``rakshastra://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``rakshastra://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"rakshastra://blueprint/{quote(blueprint.key)}{qs}"
