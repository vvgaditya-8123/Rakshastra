...
import { startGateway, stopGateway, getPort, isRunning, setLogger, GatewayInfo, healthCheck } from "./gateway";
import { BackgroundInstaller, INSTALL_STEPS } from "./installer";
...
function createWindow(): BrowserWindow {
  ...
  win.webContents.on("will-navigate", (event, url) => {
    const serverUrl = `http://127.0.0.1:${getPort()}`;
...
function startStatusPolling(): void {
  ...
  statusInterval = setInterval(async () => {
    const port = getPort();
    try {
      const isUp = await healthCheck(port, "127.0.0.1");
      if (isUp) {
        const http = require("http");
        const req = http.get(
          { hostname: "127.0.0.1", port, path: "/api/v1/status", timeout: 2000 },
          (res: any) => {
            let data = "";
...
async function launchGateway(): Promise<void> {
  try {
    ...
    gatewayInfo = await startGateway({
...
function updateSplashStatus(text: string): void {
  if (mainWindow) {
    mainWindow.webContents
      .executeJavaScript(
        `document.getElementById('status') && (document.getElementById('status').textContent = ${JSON.stringify(text)})`
      )
      .catch(() => {});
  }
}
...
function setupIPC(): void {
  ...
  ipcMain.handle("get-gateway-info", () => {
    return {
      ...gatewayInfo,
      running: isRunning(),
  ...
  ipcMain.on("setup-start-install", () => {
  ...
  ipcMain.handle("setup-save-config", async (event, data: any) => {
    ...
    let keyVar = "";
    if (data.provider === "openai") keyVar = "OPENAI_API_KEY";
    else if (data.provider === "gemini") keyVar = "GEMINI_API_KEY";
    else if (data.provider === "anthropic") keyVar = "ANTHROPIC_API_KEY";
    else if (data.provider === "openrouter") keyVar = "OPENROUTER_API_KEY";
    else if (data.provider === "ollama-cloud") keyVar = "OLLAMA_API_KEY";
    else if (data.provider === "deepseek") keyVar = "DEEPSEEK_API_KEY";
    else if (data.provider === "custom") keyVar = "CUSTOM_API_KEY";
