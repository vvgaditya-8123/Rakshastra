"""Windows UTF-8 bootstrap for Rakshastra entry points.
...
should be imported at the very top of every Rakshastra entry point
(``rakshastra``, ``rakshastra-agent``, ``rakshastra-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Rakshastra modules.

    Rakshastra ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Rakshastra source root
    ...
    """
    root = src_root or os.environ.get("RAKSHASTRA_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``RAKSHASTRA_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("RAKSHASTRA_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import rakshastra_bootstrap``
# (or ``from rakshastra_bootstrap import apply_windows_utf8_bootstrap``) at
