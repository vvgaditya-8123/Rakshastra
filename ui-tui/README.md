...
React + Ink terminal UI for Rakshastra. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `RAKSHASTRA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `rakshastra setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.rakshastra/.rakshastra_history` or under `RAKSHASTRA_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `rakshastra setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/rakshastra-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external rakshastra setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.rakshastra/tui-parent.log
    ...
    types/
      rakshastra-ink.d.ts            type declarations for @rakshastra/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent RakshastraCLI subprocess for slash commands
```