import type { RunExternalProcess } from '@rakshastra/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`rakshastra ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching rakshastra: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`rakshastra ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
