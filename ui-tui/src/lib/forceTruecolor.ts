...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.RAKSHASTRA_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Rakshastra on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via RAKSHASTRA_TUI_TRUECOLOR=1.
