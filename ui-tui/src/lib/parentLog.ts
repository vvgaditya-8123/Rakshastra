...
const logDir = join(process.env.RAKSHASTRA_HOME?.trim() || join(homedir(), '.rakshastra'), 'logs')
...
// a real ~/.rakshastra (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('rakshastra-tui: parent lifecycle log unavailable\n')
    }
  }
}
