...
Backend is selected during ``rakshastra tools`` setup (web.backend in config.yaml).
When available, Rakshastra can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Rakshastra config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Rakshastra' config/.env layer (``rakshastra config set``, ``rakshastra tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.rakshastra/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``rakshastra tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `rakshastra tools` repaint.
        try:
...
def _store_full_text(url: str, content: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_rakshastra_dir("cache/web", "web_cache")
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `rakshastra tools` to set one up."
                ),
            }
        else:
            logger.info(
