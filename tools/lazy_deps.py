"""
Lazy dependency installer for opt-in Rakshastra Agent backends.

Many Rakshastra features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``rakshastra-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``rakshastra tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``RAKSHASTRA_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/rakshastra`` read-only), setting
  ``RAKSHASTRA_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Rakshastra, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (RAKSHASTRA_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # Dashboard (`rakshastra dashboard`)
    "tool.dashboard": (
        "fastapi==0.133.1",
...
_LAZY_TARGET_ENV = "RAKSHASTRA_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``RAKSHASTRA_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("RAKSHASTRA_DISABLE_LAZY_INSTALLS") == "1":
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``rakshastra update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="rakshastra-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``rakshastra_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = rakshastra_subprocess_env(inherit_credentials=False)
...
def active_features() -> list[str]:
    ...
    Used by ``rakshastra update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``rakshastra update``. Never raises; lazy-install failures
