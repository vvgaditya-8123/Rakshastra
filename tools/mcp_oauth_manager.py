...
`mcp_tool.py`, and `rakshastra_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# RakshastraMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_rakshastra_provider_class() -> Optional[type]:
    ...
    class RakshastraMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._rakshastra_server_name = server_name
            ...
            self._rakshastra_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`RakshastraTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, RakshastraTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._rakshastra_server_name, exc,
                    )
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._rakshastra_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import RakshastraTokenStorage
                        if isinstance(storage, RakshastraTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._rakshastra_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import RakshastraTokenStorage
            if not isinstance(storage, RakshastraTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``rakshastra mcp reauth``.
            ...
            back to ``rakshastra mcp reauth``.
            """
            try:
                if self._rakshastra_preregistered:
                    return
    ...
    return RakshastraMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_RAKSHASTRA_PROVIDER_CLS: Optional[type] = _make_rakshastra_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`RakshastraMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _RAKSHASTRA_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = RakshastraTokenStorage(server_name)

        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                "MCP OAuth for "
                f"'{server_name}': non-interactive environment and no "
                "cached tokens found. Run `rakshastra mcp login "
        ...
        return _RAKSHASTRA_PROVIDER_CLS(
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``rakshastra mcp remove <name>`` and (indirectly) by
        ``rakshastra mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
