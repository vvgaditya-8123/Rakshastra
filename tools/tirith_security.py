...
it is automatically downloaded from GitHub releases to $RAKSHASTRA_HOME/bin/tirith.
...
def _get_rakshastra_home() -> str:
    """Return the Rakshastra home directory, respecting RAKSHASTRA_HOME env var."""
    return str(get_rakshastra_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_rakshastra_home(), ".tirith-install-failed")
...
def _rakshastra_bin_dir() -> str:
    """Return $RAKSHASTRA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_rakshastra_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $RAKSHASTRA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_rakshastra_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $RAKSHASTRA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $RAKSHASTRA_HOME/bin/tirith
    ...
    rakshastra_bin = os.path.join(_rakshastra_bin_dir(), "tirith")
    if os.path.isfile(rakshastra_bin) and os.access(rakshastra_bin, os.X_OK):
        _resolved_path = rakshastra_bin
        _install_failure_reason = ""
        _clear_install_failed()
