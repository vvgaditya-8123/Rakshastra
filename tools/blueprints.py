...
`rakshastra skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.rakshastra.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.rakshastra.blueprint`` is a mapping containing
    ...
    rakshastra = meta.get("rakshastra") if isinstance(meta, dict) else None
    blueprint = rakshastra.get("blueprint") if isinstance(rakshastra, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.rakshastra.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.rakshastra.blueprint`` block) they can hand
    to ``rakshastra skills publish`` to share. ``body`` is the plain-language
