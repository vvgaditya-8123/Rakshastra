#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Rakshastra Skills Hub.
...
Used by rakshastra_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _rakshastra_home() -> Path:
    return get_rakshastra_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _rakshastra_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            rakshastra_meta = metadata.get("rakshastra", {})
            if isinstance(rakshastra_meta, dict):
                tags = rakshastra_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            rakshastra_meta = metadata.get("rakshastra", {})
            if isinstance(rakshastra_meta, dict):
                raw_tags = rakshastra_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Rakshastra) but not activated
    ...
    OFFICIAL_REPO = "NousResearch/rakshastra-agent"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                rakshastra_meta = meta_block.get("rakshastra", {})
                if isinstance(rakshastra_meta, dict):
                    tags = rakshastra_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Rakshastra centralized index source
# ---------------------------------------------------------------------------

RAKSHASTRA_INDEX_URL = "https://rakshastra-agent.nousresearch.com/docs/api/skills-index.json"
RAKSHASTRA_INDEX_TTL = 6 * 3600  # 6 hours


def _rakshastra_index_cache_file() -> Path:
    return _index_cache_dir() / "rakshastra-index.json"


def _load_rakshastra_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for RAKSHASTRA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    rakshastra_index_cache_file = _rakshastra_index_cache_file()
    if rakshastra_index_cache_file.exists():
        try:
            age = time.time() - rakshastra_index_cache_file.stat().st_mtime
            if age < RAKSHASTRA_INDEX_TTL:
                return json.loads(rakshastra_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(RAKSHASTRA_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Rakshastra index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Rakshastra index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        rakshastra_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        rakshastra_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class RakshastraIndexSource(SkillSource):
    """Skill source backed by the centralized Rakshastra Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_rakshastra_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "rakshastra-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "rakshastra-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        RakshastraIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "rakshastra-index"
