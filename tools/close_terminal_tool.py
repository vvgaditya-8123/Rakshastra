#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Rakshastra desktop GUI.
...
``read_terminal`` it is gated on ``RAKSHASTRA_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — RAKSHASTRA_DESKTOP is set on the gateway the app spawns."""
    return (os.getenv("RAKSHASTRA_DESKTOP") or "").strip().lower() in ("1", "true", "yes")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Rakshastra desktop GUI (the tabs mirroring terminal(background=true) runs). "
