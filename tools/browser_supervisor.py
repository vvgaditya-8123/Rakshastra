...
One ``CDPSupervisor`` runs per Rakshastra ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Rakshastra logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "rakshastra-dialog-bridge.invalid"
...
(() => {
  if (window.__rakshastraDialogBridgeInstalled) return;
  window.__rakshastraDialogBridgeInstalled = true;
  const ENDPOINT = "http://rakshastra-dialog-bridge.invalid/";
