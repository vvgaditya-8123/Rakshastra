...
# (RAKSHASTRA_HOME env var changes) are always respected.  The old module-level
# constant was cached at import time and could go stale if a profile switch
# happened after the first import.
def get_memory_dir() -> Path:
    """Return the profile-scoped memories directory."""
    return get_rakshastra_home() / "memories"
...
class MemoryStore:
    ...
    def __init__(self, memory_char_limit: int = 2200, user_char_limit: int = 1375):
        ...
        self.security_entries: List[str] = []
        self.incident_entries: List[str] = []
        self.asset_entries: List[str] = []
        self.memory_char_limit = memory_char_limit
        self.user_char_limit = user_char_limit
        # Security/Incident/Asset share a sensible limit or have their own default limits
        self.security_char_limit = 4000
        self.incident_char_limit = 4000
        self.asset_char_limit = 5000
    ...
    def load_from_disk(self):
        ...
        self.security_entries = self._read_file(mem_dir / "SECURITY.md")
        self.incident_entries = self._read_file(mem_dir / "INCIDENT.md")
        self.asset_entries = self._read_file(mem_dir / "ASSET.md")

        # Deduplicate entries (preserves order, keeps first occurrence)
        self.memory_entries = list(dict.fromkeys(self.memory_entries))
        self.user_entries = list(dict.fromkeys(self.user_entries))
        self.security_entries = list(dict.fromkeys(self.security_entries))
        self.incident_entries = list(dict.fromkeys(self.incident_entries))
        self.asset_entries = list(dict.fromkeys(self.asset_entries))

        # Sanitize entries for the system-prompt snapshot only.  Live state
        # (memory_entries / user_entries / etc) keeps the raw text so the user
        # can see + remove poisoned entries via the memory tool.
        sanitized_memory = self._sanitize_entries_for_snapshot(self.memory_entries, "MEMORY.md")
        sanitized_user = self._sanitize_entries_for_snapshot(self.user_entries, "USER.md")
        sanitized_security = self._sanitize_entries_for_snapshot(self.security_entries, "SECURITY.md")
        sanitized_incident = self._sanitize_entries_for_snapshot(self.incident_entries, "INCIDENT.md")
        sanitized_asset = self._sanitize_entries_for_snapshot(self.asset_entries, "ASSET.md")

        # Capture frozen snapshot for system prompt injection
        self._system_prompt_snapshot = {
            "memory": self._render_block("memory", sanitized_memory),
            "user": self._render_block("user", sanitized_user),
            "security": self._render_block("security", sanitized_security),
            "incident": self._render_block("incident", sanitized_incident),
            "asset": self._render_block("asset", sanitized_asset),
        }
    ...
    def _path_for(target: str) -> Path:
        ...
        if target == "security":
            return mem_dir / "SECURITY.md"
        if target == "incident":
            return mem_dir / "INCIDENT.md"
        if target == "asset":
            return mem_dir / "ASSET.md"
    ...
    def _entries_for(self, target: str) -> List[str]:
        ...
        if target == "security":
            return self.security_entries
        if target == "incident":
            return self.incident_entries
        if target == "asset":
            return self.asset_entries
    ...
    def _set_entries(self, target: str, entries: List[str]):
        if target == "user":
            ...
            self.security_entries = entries
        elif target == "incident":
            self.incident_entries = entries
        elif target == "asset":
    ...
    def _char_limit(self, target: str) -> int:
        ...
        if target == "security":
            return self.security_char_limit
        if target == "incident":
            return self.incident_char_limit
        if target == "asset":
            return self.asset_char_limit
    ...
    def _render_block(self, target: str, entries: List[str]) -> str:
        ...
        if target == "user":
            ...
            header = f"SECURITY MEMORY (discovered vulnerabilities, compliance, and configurations) [{pct}% — {current:,}/{limit:,} chars]"
        elif target == "incident":
            header = f"INCIDENT MEMORY (security events, alerts, and investigations) [{pct}% — {current:,}/{limit:,} chars]"
        elif target == "asset":
            header = f"ASSET GRAPH / KNOWLEDGE GRAPH (servers, containers, users, secrets, and networks) [{pct}% — {current:,}/{limit:,} chars]"
        else:
...
MEMORY_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "target": {
                "type": "string",
                "enum": ["memory", "user", "security", "incident", "asset"],
                "description": "Which memory store: 'memory' for personal notes, 'user' for user profile, 'security' for security memory, 'incident' for incident logs, 'asset' for asset graph nodes."
            },
            "content": {
                "type": "string",
