...
Optional Rakshastra observability plugin that maps Rakshastra observer hooks to
...
not replace Rakshastra Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Rakshastra emit NeMo Relay lifecycle events for
the work Rakshastra already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Rakshastra Agent can:

- Preserve Rakshastra execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
rakshastra plugins enable observability/nemo_relay
```

The `RAKSHASTRA_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `RAKSHASTRA_HOME` that the
agent run will use:

```bash
env RAKSHASTRA_HOME=/tmp/rakshastra-nemo-relay-test \
  rakshastra plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`RAKSHASTRA_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`RAKSHASTRA_HOME` is the Rakshastra profile/config home used by both
`rakshastra plugins enable ...` and the later `rakshastra chat ...` run. If unset,
Rakshastra uses the user's default home, usually `~/.rakshastra`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export RAKSHASTRA_HOME=/tmp/rakshastra-nemo-relay-test
rakshastra plugins enable observability/nemo_relay
rakshastra chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `rakshastra` command you run is built from the
...
uv run rakshastra plugins enable observability/nemo_relay
uv run rakshastra chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/rakshastra_agent-*.whl
...
The plugin can configure exporters directly from `RAKSHASTRA_NEMO_RELAY_*`
...
```bash
export RAKSHASTRA_NEMO_RELAY_ATOF_ENABLED=1
export RAKSHASTRA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export RAKSHASTRA_NEMO_RELAY_ATIF_ENABLED=1
export RAKSHASTRA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `RAKSHASTRA_NEMO_RELAY_ATOF_FILENAME`
- `RAKSHASTRA_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `RAKSHASTRA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `RAKSHASTRA_NEMO_RELAY_ATIF_AGENT_NAME`
- `RAKSHASTRA_NEMO_RELAY_ATIF_AGENT_VERSION`
- `RAKSHASTRA_NEMO_RELAY_ATIF_MODEL_NAME`
- `RAKSHASTRA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Rakshastra at it:

```bash
export RAKSHASTRA_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Rakshastra Agent"
agent_version = "local"
```

When `RAKSHASTRA_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`RAKSHASTRA_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`RAKSHASTRA_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Rakshastra does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Rakshastra keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Rakshastra routes LLM and tool
...
For the full generic Rakshastra middleware contract, see
...
export RAKSHASTRA_HOME=/tmp/rakshastra-nemo-relay-docs/rakshastra-home
mkdir -p "$RAKSHASTRA_HOME"

cat > "$RAKSHASTRA_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Rakshastra session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export RAKSHASTRA_NEMO_RELAY_ATOF_ENABLED=1
export RAKSHASTRA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/rakshastra-nemo-relay-docs/subagent/atof
export RAKSHASTRA_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export RAKSHASTRA_NEMO_RELAY_ATOF_MODE=overwrite
export RAKSHASTRA_NEMO_RELAY_ATIF_ENABLED=1
export RAKSHASTRA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/rakshastra-nemo-relay-docs/subagent/atif
export RAKSHASTRA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export RAKSHASTRA_NEMO_RELAY_ATIF_AGENT_NAME='Rakshastra Agent E2E'
export RAKSHASTRA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export RAKSHASTRA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"rakshastra.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Rakshastra Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Rakshastra dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/rakshastra-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/rakshastra-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/rakshastra-nemo-relay-docs/workdir/beta.txt
cd /tmp/rakshastra-nemo-relay-docs/workdir

export RAKSHASTRA_NEMO_RELAY_ATOF_ENABLED=1
export RAKSHASTRA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/rakshastra-nemo-relay-docs/parallel/atof
export RAKSHASTRA_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export RAKSHASTRA_NEMO_RELAY_ATOF_MODE=overwrite
export RAKSHASTRA_NEMO_RELAY_ATIF_ENABLED=1
export RAKSHASTRA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/rakshastra-nemo-relay-docs/parallel/atif
export RAKSHASTRA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export RAKSHASTRA_NEMO_RELAY_ATIF_AGENT_NAME='Rakshastra Agent E2E'
export RAKSHASTRA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Rakshastra sessions map to `agent` scopes.
- Rakshastra API request hooks map to `llm` scope start/end events.
- Rakshastra tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Rakshastra execution middleware to hand
...
Enable it for Rakshastra:

```bash
export RAKSHASTRA_NEMO_RELAY_PLUGINS_TOML=/tmp/rakshastra-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Rakshastra routes execution
through these boundaries:

```text
Rakshastra provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Rakshastra provider adapter next_call(...)

Rakshastra tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Rakshastra tool dispatcher next_call(...)
```
...
middleware for a local Rakshastra run. This path requires a NeMo Relay runtime that
...
```bash
export RAKSHASTRA_HOME=/tmp/rakshastra-middleware-test/rakshastra-home
mkdir -p "$RAKSHASTRA_HOME" /tmp/rakshastra-middleware-test/nemo-relay
...
cat > /tmp/rakshastra-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/rakshastra-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/rakshastra-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Rakshastra Middleware E2E"
agent_version = "local"
...
export RAKSHASTRA_NEMO_RELAY_PLUGINS_TOML=/tmp/rakshastra-middleware-test/nemo-relay/plugins.toml
...
```json
{
  ...
  "session_id": "middleware-demo-session",
  "agent": {
    "name": "Rakshastra Middleware E2E",
    "version": "local",
