/**
 * Rakshastra Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.rakshastra/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__RAKSHASTRA_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__RAKSHASTRA_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "rakshastra-kanban-dot-triage",
    todo: "rakshastra-kanban-dot-todo",
    ready: "rakshastra-kanban-dot-ready",
    running: "rakshastra-kanban-dot-running",
    blocked: "rakshastra-kanban-dot-blocked",
    done: "rakshastra-kanban-dot-done",
    archived: "rakshastra-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-rakshastra-task";
  ...
  const DOCS_URL = "https://rakshastra-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://rakshastra-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "rakshastra.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="rakshastra-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])rakshastra-kanban-md-code\1/i.test(attrs)) {
      return ' class="rakshastra-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "rakshastra-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "rakshastra-kanban-md",
  ...
  // standard `drop` event and our `rakshastra-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("rakshastra-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("rakshastra-kanban-column--drop");
          if (target) target.classList.add("rakshastra-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("rakshastra-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("rakshastra-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("rakshastra-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__RAKSHASTRA_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "rakshastra-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "rakshastra-kanban-attention",
        "rakshastra-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "rakshastra-kanban-attention-bar" },
        h("span", { className: "rakshastra-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "rakshastra-kanban-attention-text" },
          diagTasks.length === 1
        ...
        h("button", {
          className: "rakshastra-kanban-attention-toggle",
          onClick: function () { setExpanded(function (x) { return !x; }); },
          type: "button",
        }, expanded ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
        h("button", {
          className: "rakshastra-kanban-attention-dismiss",
      ...
      expanded
        ? h("div", { className: "rakshastra-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "rakshastra-kanban-attention-row",
                  "rakshastra-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "rakshastra-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "rakshastra-kanban-attention-row-id" }, task.id),
                h("span", { className: "rakshastra-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "rakshastra-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "rakshastra-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "rakshastra-kanban-diag-action-btn",
      action.suggested ? "rakshastra-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " rakshastra-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".rakshastra-kanban-drawer-comment-row input, .rakshastra-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "rakshastra-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("rakshastra-kanban-diag", sevClass) },
      h("div", { className: "rakshastra-kanban-diag-header" },
        h("span", { className: "rakshastra-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "rakshastra-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "rakshastra-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "rakshastra-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "rakshastra-kanban-diag-data-row" },
                  h("span", { className: "rakshastra-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "rakshastra-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "rakshastra-kanban-diag-data-row" },
                h("span", { className: "rakshastra-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "rakshastra-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "rakshastra-kanban-diag-reassign-row" },
            h("span", { className: "rakshastra-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "rakshastra-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "rakshastra-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "rakshastra-kanban-diag-msg",
              msg.ok ? "rakshastra-kanban-diag-msg--ok" : "rakshastra-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "rakshastra-kanban-section" },
      h("div", { className: "rakshastra-kanban-section-head-row" },
        h("span", { className: "rakshastra-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "rakshastra-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "rakshastra-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "rakshastra-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "rakshastra-kanban-docs-link",
      title: "Open Rakshastra Kanban docs in a new tab",
      "aria-label": "Rakshastra Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `rakshastra kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "rakshastra-kanban-msg-ok" : "rakshastra-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "rakshastra-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "rakshastra-kanban-boardswitcher" },
      h("div", { className: "rakshastra-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "rakshastra-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "rakshastra-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "rakshastra-kanban-dialog-title" },
        ...
        h("div", { className: "rakshastra-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Rakshastra profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "rakshastra-kanban-bulk" },
      h("span", { className: "rakshastra-kanban-bulk-count" },
      ...
      h("div", { className: "rakshastra-kanban-bulk-priority",
      ...
      h("div", { className: "rakshastra-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Rakshastra profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "rakshastra-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("rakshastra-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("rakshastra-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "rakshastra-kanban-trash",
        dragOver ? "rakshastra-kanban-trash--drop" : "",
        props.draggingTaskId ? "rakshastra-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "rakshastra-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "rakshastra-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".rakshastra-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "rakshastra-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("rakshastra-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("rakshastra-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "rakshastra-kanban-column",
        dragOver ? "rakshastra-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "rakshastra-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "rakshastra-kanban-col-check",
        ...
        h("span", { className: cn("rakshastra-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "rakshastra-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "rakshastra-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "rakshastra-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "rakshastra-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "rakshastra-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "rakshastra-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "rakshastra-kanban-lane" },
                  h("div", { className: "rakshastra-kanban-lane-head" },
                    h("span", { className: "rakshastra-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "rakshastra-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "rakshastra-kanban-card--stale-red";
    if (age >= tier.amber) return "rakshastra-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".rakshastra-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "rakshastra-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "rakshastra-kanban-card",
        props.selected ? "rakshastra-kanban-card--selected" : "",
        props.failed ? "rakshastra-kanban-card--failed" : "",
        props.draggingSource ? "rakshastra-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "rakshastra-kanban-card-content" },
          h("div", { className: "rakshastra-kanban-card-row" },
            h("label", {
              className: "rakshastra-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "rakshastra-kanban-card-check",
            ...
            h("span", { className: "rakshastra-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or rakshastra kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "rakshastra-kanban-warning-badge",
                    "rakshastra-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
              ...
              : null,
            t.priority > 0
              ? h(Badge, { className: "rakshastra-kanban-priority",
                           title: `Priority ${t.priority}. Higher-priority tasks are claimed first by the dispatcher.` }, `P${t.priority}`)
              : null,
            t.tenant
              ? h(Badge, { variant: "outline", className: "rakshastra-kanban-tag",
                           title: `Tenant: ${t.tenant}. Free-form tag for grouping tasks (customer, project, team).` }, t.tenant)
              : null,
            progress
              ? h("span", {
                  className: cn(
                    "rakshastra-kanban-progress",
                    progress.done === progress.total ? "rakshastra-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "rakshastra-kanban-needs-assignee",
          ...
          h("div", { className: "rakshastra-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "rakshastra-kanban-card-row rakshastra-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "rakshastra-kanban-assignee",
                            title: `Assigned to Rakshastra profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "rakshastra-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "rakshastra-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "rakshastra-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "rakshastra-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Rakshastra profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Rakshastra profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "rakshastra-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "rakshastra-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "rakshastra-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "rakshastra-kanban-drawer-close",
        ...
        data ? h("div", { className: "rakshastra-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "rakshastra-kanban-section" },
      h("div", { className: "rakshastra-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "rakshastra-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "rakshastra-kanban-drawer-body" },
      h("div", { className: "rakshastra-kanban-drawer-title" },
        h("span", { className: cn("rakshastra-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "rakshastra-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "rakshastra-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "rakshastra-kanban-section" },
        h("div", { className: "rakshastra-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "rakshastra-kanban-section" },
        h("div", { className: "rakshastra-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "rakshastra-kanban-comment" },
                h("div", { className: "rakshastra-kanban-comment-head" },
                  h("span", { className: "rakshastra-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "rakshastra-kanban-comment-ago" },
      ...
      h("div", { className: "rakshastra-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "rakshastra-kanban-event",
              isDiag ? "rakshastra-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "rakshastra-kanban-event-header" },
                  h("span", { className: "rakshastra-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "rakshastra-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "rakshastra-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "rakshastra-kanban-event-header-plain" },
                  h("span", { className: "rakshastra-kanban-event-kind" }, e.kind),
                  h("span", { className: "rakshastra-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "rakshastra-kanban-event-phantom-row" },
                  h("span", { className: "rakshastra-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "rakshastra-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "rakshastra-kanban-event-payload" },
    ...
    return h("div", { className: "rakshastra-kanban-section" },
      h("div", { className: "rakshastra-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "rakshastra-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `rakshastra-kanban-run--${r.outcome || r.status || "ended"}`
          : "rakshastra-kanban-run--active";
        return h("div", { key: r.id, className: cn("rakshastra-kanban-run", outcomeClass) },
          h("div", { className: "rakshastra-kanban-run-head" },
            h("span", { className: "rakshastra-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "rakshastra-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "rakshastra-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "rakshastra-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "rakshastra-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "rakshastra-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "rakshastra-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "rakshastra-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "rakshastra-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "rakshastra-kanban-pre rakshastra-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "rakshastra-kanban-meta-row" },
      h("span", { className: "rakshastra-kanban-meta-label" }, props.label),
      h("span", { className: "rakshastra-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "rakshastra-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "rakshastra-kanban-meta-row" },
        h("span", { className: "rakshastra-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "rakshastra-kanban-meta-value rakshastra-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "rakshastra-kanban-meta-row" },
        h("span", { className: "rakshastra-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "rakshastra-kanban-meta-value rakshastra-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "rakshastra-kanban-section" },
      h("div", { className: "rakshastra-kanban-section-head-row" },
        h("span", { className: "rakshastra-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "rakshastra-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "rakshastra-kanban-section" },
      h("div", { className: "rakshastra-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "rakshastra-kanban-deps-row" },
        h("span", { className: "rakshastra-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "rakshastra-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "rakshastra-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "rakshastra-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "rakshastra-kanban-dep-chip-x",
      ...
      h("div", { className: "rakshastra-kanban-deps-row" },
        h("span", { className: "rakshastra-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "rakshastra-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "rakshastra-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "rakshastra-kanban-msg-ok"
          : "rakshastra-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "rakshastra-kanban-section" },
      ...
      h("div", { className: "rakshastra-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "rakshastra-kanban-home-sub rakshastra-kanban-home-sub--on"
              : "rakshastra-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__RAKSHASTRA_PLUGINS__ && typeof window.__RAKSHASTRA_PLUGINS__.register === "function") {
    window.__RAKSHASTRA_PLUGINS__.register("kanban", KanbanPage);
  }
})();
