...
session-scoped state file under ``$RAKSHASTRA_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $RAKSHASTRA_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_rakshastra_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    guest_name: str = "Rakshastra Agent",
    ...
    If a bot is already running for this rakshastra install, leave it first —
    ...
    env = os.environ.copy()
    env["RAKSHASTRA_MEET_URL"] = url
    env["RAKSHASTRA_MEET_OUT_DIR"] = str(out)
    env["RAKSHASTRA_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["RAKSHASTRA_MEET_HEADED"] = "1"
    if auth_state:
        env["RAKSHASTRA_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["RAKSHASTRA_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if RAKSHASTRA_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["RAKSHASTRA_MEET_MODE"] = mode
    if realtime_model:
        env["RAKSHASTRA_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["RAKSHASTRA_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["RAKSHASTRA_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["RAKSHASTRA_MEET_REALTIME_KEY"] = realtime_api_key
