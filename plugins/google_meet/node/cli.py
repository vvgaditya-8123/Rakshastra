"""`rakshastra meet node ...` subcommand tree.

Wired into the existing ``rakshastra meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``rakshastra meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="rakshastra-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``rakshastra meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             rakshastra meet node approve <name> ws://<host>:{args.port} {token}")
        try:
