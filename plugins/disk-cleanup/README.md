...
Auto-tracks and cleans up ephemeral files created during Rakshastra Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$RAKSHASTRA_HOME` and `/tmp/rakshastra-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `RAKSHASTRA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under RAKSHASTRA_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `RAKSHASTRA_HOME` or `/tmp/rakshastra-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$RAKSHASTRA_HOME/disk-cleanup/` is itself excluded
- `$RAKSHASTRA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
