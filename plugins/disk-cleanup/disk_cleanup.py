"""disk_cleanup — ephemeral file cleanup for Rakshastra Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under RAKSHASTRA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly RAKSHASTRA_HOME and /tmp/rakshastra-*
Never touches: ~/.rakshastra/logs/ or any system directory.
"""
...
try:
    ...
    def get_rakshastra_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("RAKSHASTRA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".rakshastra").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$RAKSHASTRA_HOME/logs/``."""
    return get_rakshastra_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$RAKSHASTRA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under RAKSHASTRA_HOME or ``/tmp/rakshastra-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    rakshastra_home = get_rakshastra_home()
    try:
        path.resolve().relative_to(rakshastra_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/rakshastra-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("rakshastra-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "rakshastra-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $RAKSHASTRA_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so RAKSHASTRA_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        rakshastra_home = get_rakshastra_home()
        for parent in ("cron", "cronjobs"):
            base = rakshastra_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside RAKSHASTRA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under RAKSHASTRA_HOME, but never recurse into known
    # durable state trees.  Some installs place the Rakshastra checkout, venv,
    # and desktop build under RAKSHASTRA_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    rakshastra_home = get_rakshastra_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in rakshastra_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    rakshastra_home = get_rakshastra_home()
    try:
        rel = path.resolve().relative_to(rakshastra_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "rakshastra-agent",
        }:
            return None
        ...
        # Path isn't under RAKSHASTRA_HOME (e.g. /tmp/rakshastra-*) — fall through.
        pass
