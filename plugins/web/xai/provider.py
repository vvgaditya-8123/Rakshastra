...
``{title, url, description, position}`` rows every other Rakshastra web
provider produces.
...
prefers Rakshastra-managed xAI Grok OAuth (via ``rakshastra auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.rakshastra/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``rakshastra tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `rakshastra auth` to sign in with "
