...
```bash
rakshastra memory setup    # select "holographic"
```

Or manually:
```bash
rakshastra config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.rakshastra-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$RAKSHASTRA_HOME/memory_store.db` | SQLite database path |
