...
```bash
rakshastra memory setup    # select "byterover"
```

Or manually:
```bash
rakshastra config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.rakshastra/.env
```
...
Working directory: `$RAKSHASTRA_HOME/byterover/` (profile-scoped).
