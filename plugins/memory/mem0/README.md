...
```bash
rakshastra memory setup    # select "mem0"
```

Or manually:
```bash
rakshastra config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.rakshastra/.env
```

## Config

Behavioral settings live in `$RAKSHASTRA_HOME/mem0.json` (set them via `rakshastra memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.rakshastra/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `rakshastra-user` | User identifier on Mem0 |
| `agent_id` | `rakshastra` | Agent identifier |
...
```bash
rakshastra memory setup mem0 --mode oss \
...
```bash
rakshastra memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$RAKSHASTRA_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.rakshastra/mem0_qdrant"}}
  }
}
```
...
```bash
rakshastra memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
rakshastra memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$RAKSHASTRA_HOME/mem0.json`).
