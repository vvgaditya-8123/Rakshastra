...
Configuration
-------------
Secret (lives in $RAKSHASTRA_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $RAKSHASTRA_HOME/mem0.json, set via `rakshastra memory
setup`):
  ...
  agent_id           — Agent identifier (default: rakshastra)
...
_DEFAULT_USER_ID = "rakshastra-user"
...
def _load_config() -> dict:
    """Load config from env vars, with $RAKSHASTRA_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "rakshastra"),
        "oss": {},
    }
    ...
    config_path = get_rakshastra_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, rakshastra_home):
        """Write config to $RAKSHASTRA_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, rakshastra_home: str, config: dict) -> None:
        ...
        post_setup(rakshastra_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $RAKSHASTRA_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "rakshastra")
