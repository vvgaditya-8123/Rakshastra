...
```bash
rakshastra memory setup    # select "hindsight"
```
...
```bash
rakshastra config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.rakshastra/.env
```
...
Rakshastra spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.rakshastra/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p rakshastra ui start
```
...
Config file: `~/.rakshastra/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `rakshastra` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `rakshastra-{profile}` isolates memory per active Rakshastra profile. Empty placeholders collapse cleanly (e.g. `rakshastra-{user}` with no user becomes `rakshastra`). |
...
| `retain_context` | `conversation between Rakshastra Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.rakshastra/.env` as `HINDSIGHT_LLM_API_KEY`.
