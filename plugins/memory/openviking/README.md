...
```bash
rakshastra memory setup    # select "openviking"
```

The setup can link to an existing `~/.openviking/ovcli.conf`, copy its current
connection values into Rakshastra, or create a minimal `ovcli.conf` when one does
not exist.

Or manually:
```bash
rakshastra config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.rakshastra/.env
```
...
| `OPENVIKING_AGENT` | `rakshastra` | Rakshastra peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Rakshastra lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Rakshastra sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Rakshastra built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Rakshastra action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Rakshastra
...
file URIs, such as
`viking://user/peers/rakshastra/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/rakshastra/memories/preferences/mem_abc123.md`. Files
