...
- Supermemory API key from [app.supermemory.ai/integrations?connect=rakshastra](http://app.supermemory.ai/integrations?connect=rakshastra)

## Setup

```bash
rakshastra memory setup    # select "supermemory"
```

Or manually:

```bash
rakshastra config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.rakshastra/.env
```

## Config

Config file: `$RAKSHASTRA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `rakshastra` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `rakshastra-{identity}` → `rakshastra-coder`). |
...
All Supermemory API calls send `x-sm-source: rakshastra`, and document writes stamp
`metadata.sm_source: rakshastra`. This is a **functional routing key, not telemetry**:
it groups Rakshastra-written memories into a dedicated "Rakshastra" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Rakshastra profile:

```json
{
  "container_tag": "rakshastra-{identity}"
}
```

For a profile named `coder`, this resolves to `rakshastra-coder`. The default profile resolves to `rakshastra-default`. Without `{identity}`, all profiles share the same container.
