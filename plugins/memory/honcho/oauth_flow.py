...
``rakshastra://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Rakshastra OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "rakshastra-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``rakshastra-desktop`` / ``rakshastra-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Rakshastra.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local RAKSHASTRA_HOME override can't reach.
