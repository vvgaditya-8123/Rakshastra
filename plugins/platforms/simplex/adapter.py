"""SimpleX Chat platform adapter (Rakshastra plugin).
...
This adapter ships as a Rakshastra platform plugin under
``plugins/platforms/simplex/``. The Rakshastra plugin loader scans the
...
Optional environment variables:
    ...
    RAKSHASTRA_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``rakshastra setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Rakshastra — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("RAKSHASTRA_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``rakshastra setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.rakshastra/.env`` via
    ...
    try:
        ...
        print(
            "rakshastra_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.rakshastra/.env"
        )
        return
