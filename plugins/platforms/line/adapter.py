"""
LINE Messaging API platform adapter for Rakshastra Agent.
...
to Rakshastra Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``RAKSHASTRA_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            rakshastra_home = Path(get_rakshastra_home()).resolve()
        except Exception:
            rakshastra_home = Path.home().joinpath(".rakshastra").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``rakshastra status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``rakshastra status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``rakshastra setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.rakshastra/.env`` via ``rakshastra_cli.config``.
    """
    print()
    ...
    try:
        from rakshastra_cli.config import get_env_var, set_env_var
    except ImportError:
        print("rakshastra_cli.config not available; set LINE_* vars manually in ~/.rakshastra/.env")
        return
