// Rakshastra Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Rakshastra): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Rakshastra -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Rakshastra-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `rakshastra photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Rakshastra can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Rakshastra'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Rakshastra allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Rakshastra, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-rakshastra-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Rakshastra supplied them so a known-good
