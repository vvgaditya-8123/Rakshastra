...
Small Node helper that bridges Rakshastra Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Rakshastra is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Rakshastra-Sidecar-Token`)
...
The Rakshastra plugin's `rakshastra photon setup` command runs `npm install`
here automatically.
