...
Credential storage mirrors every other Rakshastra channel:

    * runtime SDK creds  -> ``~/.rakshastra/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.rakshastra/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a rakshastra dependency
...
# registers Rakshastra as its own client_id.
...
# Default name of the project Rakshastra provisions for the operator.
DEFAULT_PROJECT_NAME = "Rakshastra Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of rakshastra-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.rakshastra/auth.json`` honouring the active Rakshastra profile."""
    try:
        from rakshastra_constants import get_rakshastra_home
        return Path(get_rakshastra_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.rakshastra")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.rakshastra/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.rakshastra/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.rakshastra/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: rakshastra_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `rakshastra photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `rakshastra photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `rakshastra photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `rakshastra photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `rakshastra photon setup`)"
