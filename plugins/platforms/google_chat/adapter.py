...
# ``model_tools`` import time, so ``rakshastra status``, ``rakshastra chat``, etc.
...
# ``rakshastra_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Rakshastra is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a rakshastra MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${RAKSHASTRA_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _rakshastra_home = _get_rakshastra_home()
        except (ModuleNotFoundError, ImportError):
            _rakshastra_home = _Path.home() / ".rakshastra"
        self._thread_count_store = _ThreadCountStore(
            _rakshastra_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Rakshastra is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("RAKSHASTRA_HOME", str(_Path.home() / ".rakshastra"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${RAKSHASTRA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a rakshastra MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Rakshastra is thinking…' marker message.
        ...
        "Rakshastra is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Rakshastra is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Rakshastra is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Rakshastra is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Rakshastra is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``rakshastra setup``.

    The setup wizard at ``rakshastra_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.rakshastra/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. rakshastra-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.rakshastra/.env")
    print_info("Restart the gateway: rakshastra gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``rakshastra cron`` running as a
    separate process from ``rakshastra gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Rakshastra plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'rakshastra-agent[google_chat]'",
        ...
        # the "Rakshastra is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Rakshastra is thinking…' marker message "
