"""
Email platform adapter for the Rakshastra gateway.

Allows users to interact with Rakshastra by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            ...
            _rakshastra_version = "0"
        imap.xatom(
            "ID",
            f'("name" "rakshastra-agent" "version" "{_rakshastra_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `rakshastra config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `rakshastra gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Rakshastra Agent")
        ...
        msg_id = f"<rakshastra-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
...
# rakshastra_cli/gateway.py, and the _send_email dispatch in
