"""ntfy platform adapter (Rakshastra plugin).
...
a Rakshastra dependency.

This adapter ships as a Rakshastra platform plugin under
``plugins/platforms/ntfy/``. The Rakshastra plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "rakshastra-in"              # subscribe topic (incoming)
          publish_topic: "rakshastra-out"     # optional — defaults to topic
...
_ECHO_TAG = "rakshastra-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``rakshastra cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Rakshastra dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `rakshastra gateway status` without
