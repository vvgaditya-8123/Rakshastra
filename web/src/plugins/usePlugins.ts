...
import { api, RAKSHASTRA_BASE_PATH } from "@/lib/api";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${RAKSHASTRA_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${RAKSHASTRA_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?rakshastra_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-rakshastra-plugin", manifest.name);
