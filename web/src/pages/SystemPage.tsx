...
export default function SystemPage() {
  ...
  // spawned `rakshastra import` runs non-interactively (stdin is /dev/null), so
  ...
  const loadAll = useCallback(() => {
    Promise.allSettled([
      ...
      api.checkRakshastraUpdate(false),
    ])
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_rakshastra === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkRakshastraUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_rakshastra],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_rakshastra === false) {
      showToast(
        "Rakshastra updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      ...
      setActiveAction(resp.name ?? "rakshastra-update");
  ...
  const canUpdateRakshastra = status?.can_update_rakshastra !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateRakshastra && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Rakshastra?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'rakshastra update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'rakshastra update' (${updateInfo?.update_command ?? "rakshastra update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Rakshastra</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.rakshastra_version}</span>
                  {canUpdateRakshastra &&
                    updateInfo &&
      ...
      <section className="flex flex-col gap-3">
        <H2 variant="sm" className="flex items-center gap-2 text-muted-foreground">
          <Globe className="h-4 w-4" /> Threat Center & System Status
        </H2>
        <Card>
          <CardContent className="flex flex-col gap-3 py-4">
            <div className="flex items-center gap-3">
              <Badge tone={portal?.logged_in ? "success" : "secondary"}>
                {portal?.logged_in ? "system active" : "idle"}
              </Badge>
              {portal?.provider && (
                <span className="text-sm text-muted-foreground">
                  security scanner provider: {portal.provider}
                </span>
              )}
              <a
                href={portal?.subscription_url || "https://docs.rakshastra.local"}
                target="_blank"
                rel="noreferrer"
                className="ml-auto text-xs text-primary underline"
              >
                Security Policies & License
              </a>
            </div>
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Configure credentials with <span className="font-mono">rakshastra portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="flex flex-col gap-4 py-4">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
              ...
              <span className="ml-auto">
                New credentials:{" "}
                <span className="font-mono">rakshastra memory setup</span>
              </span>
            </div>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$RAKSHASTRA_HOME/backups/rakshastra-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Rakshastra backup?"
              description={`This will overwrite your current Rakshastra configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Rakshastra team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
