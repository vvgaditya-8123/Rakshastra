"""Centralized logging setup for Rakshastra Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.rakshastra/logs/`` (profile-aware via ``get_rakshastra_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``rakshastra_cli.web_server``,
    ``rakshastra_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Rakshastra (TUI, gateway, ``hy_memory`` server, MCP
...
from rakshastra_constants import get_config_path, get_rakshastra_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_rakshastra_session_injector", False):
    ...
    _session_record_factory._rakshastra_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``rakshastra logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``rakshastra logs --component gateway``.
    "gateway": ("gateway", "rakshastra_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("rakshastra_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "rakshastra_cli.web_server",
        "rakshastra_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    rakshastra_home: Optional[Path] = None,
    ...
    """Configure the Rakshastra logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    rakshastra_home
        Override for the Rakshastra home directory.  Falls back to
        ``get_rakshastra_home()`` (profile-aware).
    log_level
    ...
    home = rakshastra_home or get_rakshastra_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_rakshastra_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._rakshastra_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the rakshastra group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from rakshastra_cli.config import is_managed
